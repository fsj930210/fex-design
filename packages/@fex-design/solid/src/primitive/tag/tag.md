...
| content | `children?: JSX.Element` | — | Tag content. |
...
Content follows Solid's native model: `children?: JSX.Element`. Place `TagClose` inside `Tag` when Primitive composition needs a close control. `TagClose` does not remove its parent and does not coordinate the parent disabled state.
