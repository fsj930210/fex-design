...
const item = inject(anchorItemContextKey)
...
<template>
  <button
    ...
    :data-state="anchor.activeKeys.value.includes(item.key) ? 'active' : 'inactive'"
    :class="
      cn(
        anchorLinkClassName({
          orientation: anchor.orientation.value,
          active: anchor.highlightedKeys.value.has(item.key),
        }),
