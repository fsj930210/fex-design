...
import { Tooltip } from '@fex-design/vue/ui/tooltip'
    ...
    <div class="grid w-full gap-4 overflow-visible">
      ...
      >
        <div v-for="item in cases" :key="item.placement" :style="{ gridArea: item.cell }">
          <Tooltip :placement="item.placement" :title="item.placement" :avoid-collisions="false"
            ><Button variant="outlined">{{ item.placement }}</Button></Tooltip
          >
        </div>
      </div>
    </div>
