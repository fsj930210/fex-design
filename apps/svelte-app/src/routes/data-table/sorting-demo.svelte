...
{#snippet header(item: Header<Features, Person>, revision: number)}
  ...
  <DataTableSortButton column={item.column}
    >{String(item.column.columnDef.header)}</DataTableSortButton
  >
{/snippet}
...
>
  <div class="grid gap-3 xl:grid-cols-2">
    {#each grids as item (item.title)}<section class="space-y-1.5">
        ...
        <DataTable table={item.table} {header} /><ReactiveTableText
          snapshot={item.table.dataTableSnapshot}
          text={(revision) => remoteText(item, revision)}
        />
      </section>{/each}
  </div>
</DemoSection>
