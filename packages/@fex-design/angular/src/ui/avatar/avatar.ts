...
import { ChangeDetectionStrategy, Component, ElementRef, TemplateRef, contentChildren, computed, effect, inject, input } from '@angular/core'
...
@Component({ selector: 'span[avatar]', standalone: true, imports: [PrimitiveAvatar, AvatarImage, AvatarFallback], changeDetection: ChangeDetectionStrategy.OnPush, templateUrl: './avatar.html' })
export class Avatar {
...
@Component({ selector: 'div[avatarGroup]', standalone: true, imports: [PrimitiveAvatarGroup, AvatarGroupCount, NgTemplateOutlet], changeDetection: ChangeDetectionStrategy.OnPush, templateUrl: './avatar-group.html' })
export class AvatarGroup {
  ...
  constructor() { effect(() => { const visible = new Set(this.split().visibleItems); for (const avatar of this.avatars()) avatar.element.nativeElement.hidden = !visible.has(avatar) }) }
}
