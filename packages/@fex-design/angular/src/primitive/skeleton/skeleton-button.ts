...
@Component({
  selector: 'div[skeletonButton]',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: { '[class]': 'hostClassName()', 'aria-hidden': 'true', 'data-slot': 'skeleton-button' },
  template: '',
})
export class SkeletonButton {
  ...
  readonly shape = input<SkeletonButtonShape | undefined>()
  readonly size = input<SkeletonButtonSize>('md')
  protected readonly hostClassName = createHostClassName(() =>
    cn(
      skeletonBaseClassName,
      skeletonBlockClassName,
      skeletonAnimationVariants({ animation: this.animation() }),
      skeletonButtonClassName({ block: this.block(), shape: this.shape(), size: this.size() }),
      this.className(),
    ),
  )
}
