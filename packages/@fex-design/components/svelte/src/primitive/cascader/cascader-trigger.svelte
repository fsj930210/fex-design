<script lang="ts">
  ...
  function keydown(event: KeyboardEvent) {
    if (event.key === "ArrowDown" || event.key === "ArrowUp")
      ...
      cascader.controller.moveToBoundary(
        event.key === "Home" ? "first" : "last",
      );
    else if (event.key === "Enter" || event.key === " ")
      cascader.controller.selectActive();
...
<PopoverTrigger
  >{#snippet children(slot)}<div
      ...
      class={cn(cascaderTriggerClassName(), className)}
    ...
    >
      <div class={cascaderValueContainerClassName}>
        <CascaderValue /><input
          ...
          placeholder={cascader.showSearch() && !selected().length
          ...
          oninput={(event) => {
            ...
            keyword.trim()
              ? cascader.controller.open()
              : cascader.controller.close();
          }}
        />
      </div>
      <span class={cascaderSuffixClassName}
        >{#if cascader.loading()}<LoadingIcon
            class="animate-spin"
          />{:else if cascader.clearable() && selected().length}<Button
          ...
          >{:else}<span
            ...
            ><ChevronRight class="rotate-90" /></span
          >{/if}</span
      >
