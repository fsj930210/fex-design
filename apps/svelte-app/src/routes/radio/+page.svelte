    ...
    <header class="space-y-4">
      ...
      <div>
        ...
        <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Single-value radio group with native aria state and button-style radio
          options.
        </p>
      </div>
    </header>
    ...
    <div class="space-y-4">
      ...
      >
        <div class="grid min-w-0 gap-2">
          <RadioGroup
            value={controlled}
            onValueChange={(value) => (controlled = value)}
      ...
      >
        <div class="grid min-w-0 gap-2">
          <RadioGroup
            value={groupValue}
            onValueChange={(value) => (groupValue = value)}
          >
            {#each options as option (option.value)}
              <label class="inline-flex min-w-0 items-center gap-2 text-sm">
                <Radio value={option.value} />
      ...
      >
        <div class="grid min-w-0 gap-2">
          <RadioGroup
            value={buttonValue}
            onValueChange={(value) => (buttonValue = value)}
            orientation="horizontal"
      ...
      >
        <div class="grid min-w-0 gap-2">
          <RadioGroup defaultValue="pear" orientation="horizontal">
            ...
            >
            <label class="inline-flex min-w-0 items-center gap-2 text-sm"
              ><Radio value="orange" disabled /><span>Orange disabled</span
              ></label
            >
          </RadioGroup>
          <RadioGroup
