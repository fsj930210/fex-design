<script lang="ts">
  ...
  interface Props
    extends
      Omit<HTMLAttributes<HTMLSpanElement>, "class" | "color">,
      BadgeOptions {
    class?: string;
    children?: Snippet;
  }
  ...
  const visible = $derived(
    (value !== undefined && value !== null && (value !== 0 || showZero)) ||
      children !== undefined,
  );
  ...
  const classList = $derived(
    cn(badgeClassName({ color: presetColor, size }), className),
  );
  const mergedStyle = $derived(
    [style, customColor ? `--badge-color:${customColor}` : ""]
      .filter(Boolean)
      .join(";"),
  );
</script>
...
{#if visible}<span
    ...
    >{#if value !== undefined}{value}{:else}{@render children?.()}{/if}</span
  >{/if}
