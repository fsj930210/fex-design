<script lang="ts">
  ...
  let { value, defaultValue, disabled, orientation = 'vertical', onChange, class: className, children, ...rest }: Props = $props();
  const options = { get value() { return value; }, get defaultValue() { return defaultValue; }, get disabled() { return disabled; }, get onChange() { return onChange; } };
  const controller = createCheckboxGroupController(options);
  const snapshot = readableCoreStore(controller);
  setCheckboxGroupContext({ value: () => value ?? $snapshot.value, disabled: () => disabled === true, toggle: controller.toggle });
</script>

<div {...rest} role="group" data-slot="checkbox-group" data-orientation={orientation} class={cn(checkboxGroupClassName({ orientation }), className)}>{@render children?.()}</div>
