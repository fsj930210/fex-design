...
{#snippet groupRow(row: Row<Features, Person>)}<div
  ...
  >
    <Button size="icon-xs" variant="ghost" onclick={() => row.toggleExpanded()}
      ><ChevronRightIcon
        class={`size-3.5 ${row.getIsExpanded() ? "rotate-90" : ""}`}
      /></Button
    ><span class="font-semibold">{row.groupingColumnId}</span><span>=</span
    ><span>{String(row.groupingValue)}</span><span
      class="rounded-full bg-foreground/10 px-2 py-0.5 text-xs"
  ...
  ><div class="grid gap-3 xl:grid-cols-2">
    {#each grids as item (item.title)}<section class="space-y-1.5">
        ...
        <div class="flex gap-1.5">
          {#each ["department", "status"] as id (id)}<Button
              size="sm"
              variant={item.table.getColumn(id)?.getIsGrouped()
                ? "default"
                : "outline"}
              onclick={() => item.table.getColumn(id)?.toggleGrouping()}
              >{item.table.getColumn(id)?.getIsGrouped() ? "Ungroup" : "Group"}
              {id}</Button
            >{/each}
        </div>
        <DataTable table={item.table} {groupRow} />
      </section>{/each}
  </div></DemoSection
>
