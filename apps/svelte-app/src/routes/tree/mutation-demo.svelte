<script lang="ts">
  ...
  let treeData: readonly DepartmentNode[] = $state(departmentTreeData);
...
<Card
  ...
  ><div class="mb-2 flex flex-wrap gap-1.5">
    <Button
      ...
      variant="outline"
      onclick={() =>
        controller.updateNode("engineering", { name: "Engineering (renamed)" })}
    ...
    ><Button
      ...
      variant="outline"
      onclick={() =>
        controller.getFeature<FocusFeatureApi>("focus")?.reveal("design")}
  ...
  <DemoTree
    ...
    fieldNames={departmentFieldNames}
    isLeaf={(n) => n.childCount === 0}
    onTreeDataChange={(next) => (treeData = next)}
