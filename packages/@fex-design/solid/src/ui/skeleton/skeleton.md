...
## Import
    ...
    import { Skeleton, SkeletonAvatar, SkeletonBlock, SkeletonButton, SkeletonImage, SkeletonInput, SkeletonText } from '@fex-design/solid/ui/skeleton'
...
| styles | SkeletonStyles<`JSX.CSSProperties`> | — | Inline styles for root, avatar, title, and paragraph. |
| native attributes | `JSX.HTMLAttributes<HTMLDivElement>` | — | Passed to the generated root div; class/style merge with root structured options. |
...
`children` provides real content; the `placeholder` prop provides custom JSX content.
