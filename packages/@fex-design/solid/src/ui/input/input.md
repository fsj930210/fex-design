...
## Import
    ...
    import { Input, InputPassword, InputSearch, InputGroup } from '@fex-design/solid/ui/input'
...
| prefix / suffix          | Solid `JSX.Element` content props.                       | —            | Content inside InputRoot; non-interactive on regular Input.           |
| addonBefore / addonAfter | Solid `JSX.Element` content props.                       | —            | Content outside the Input focus ring.                                 |
...
| native attributes        | `JSX.InputHTMLAttributes<HTMLInputElement>`              | —            | Native input attributes and events.                                   |
...
`ref` returns the native `HTMLInputElement`.

## Styling

`class`, `classNames`, and `JSX.CSSProperties` styles. Use `classNames` / `styles` for semantic parts, selectors for hover/focus/invalid states, and base variables such as `--input-background`, `--input-border-color`, `--input-color`, `--input-placeholder-color`, `--input-ring-color`, and `--input-group-radius` for reusable theming.
