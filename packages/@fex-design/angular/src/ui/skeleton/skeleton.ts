...
import {
  ...
  SkeletonInput,
  SkeletonText,
} from '../../primitive/skeleton/skeleton'
import { createHostClassName } from '../../signals/host-class'
import { SkeletonPlaceholder } from './skeleton-placeholder'
@Component({
  selector: 'div[skeleton]',
  standalone: true,
  imports: [NgTemplateOutlet, SkeletonAvatar, SkeletonText],
  changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    '[class]': 'hostClassName()',
    '[style]': 'loading() === false || placeholder() ? null : styles().root',
    '[attr.aria-hidden]': 'loading() === false ? null : true',
...
export class Skeleton {
  ...
  protected readonly hostClassName = createHostClassName(() =>
    this.loading() === false || this.placeholder()
      ? 'contents'
      : cn(skeletonRootClassName, this.classNames().root),
  )
  protected readonly avatarOptions = () =>
    typeof this.avatar() === 'object' ? (this.avatar() as SkeletonAvatarOptions) : {}
  protected readonly titleOptions = () =>
    typeof this.title() === 'object' ? (this.title() as SkeletonTitleOptions) : {}
  protected readonly paragraphOptions = () =>
    typeof this.paragraph() === 'object' ? (this.paragraph() as SkeletonParagraphOptions) : {}
  ...
  protected widthStyle(width: SkeletonWidth | undefined, style: string | undefined) {
    const value =
      width === undefined ? '' : `width: ${typeof width === 'number' ? `${width}px` : width}`
    return [value, style].filter(Boolean).join('; ')
  }
  protected rowWidth(index: number) {
    const options = this.paragraphOptions()
    return Array.isArray(options.width)
      ? options.width[index]
      : index === this.rows() - 1
        ? options.width
        : undefined
