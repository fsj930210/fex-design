<script lang="ts">
  ...
  function choose(result: DemoTreeSearchResult) {
    selected = result.node.id;
    keyword = "";
    void getDemoTreeSubtree(result.node.id).then(
      (value) => (treeData = convert(value.treeData)),
    );
  }
  function clear() {
    keyword = "";
...
>
  <InputRoot
    ...
    onClear={clear}
    class="mb-2 max-w-sm"
    ><InputControl placeholder="Search remote departments" /><InputClear
    /></InputRoot
  >
  {#if keyword}<Listbox
      items={results}
      getItemValue={(item: unknown) => (item as DemoTreeSearchResult).node.id}
      >{#each results as result (result.node.id)}<ListboxItem
          ...
          ></ListboxItem
        >{/each}</Listbox
    >{:else if treeData.length}<DemoTree
      {treeData}
