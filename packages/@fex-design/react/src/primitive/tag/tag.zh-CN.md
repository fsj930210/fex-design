...
| 事件 | `onClick` / native mouse events | — | 原生按钮事件；Primitive 不增加专用关闭事件。 |
...
内容遵循 React 原生模型：`children?: ReactNode`。Primitive 需要关闭能力时，在 `Tag` 内组合 `TagClose`。`TagClose` 不会删除父标签，也不会自动联动父级 disabled。
