...
const emit = defineEmits<{
  change: [keys: readonly string[], items: readonly AnchorRegisteredItem[]]
}>()
...
const anchor = useAnchor({
  ...
  container: () => (typeof props.container === 'function' ? props.container() : props.container),
...
const rootClass = computed(() =>
  cn(anchorRootClassName({ orientation: props.orientation }), attrs.class as string | undefined),
)
</script>
<template>
  <nav
    v-bind="attrs"
    :ref="(element) => (anchor.root.value = element as HTMLElement)"
    data-slot="anchor"
