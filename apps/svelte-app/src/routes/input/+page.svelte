  ...
  <div class="mx-auto w-full max-w-5xl space-y-4">
    <header class="space-y-2">
      ...
      <p class="max-w-2xl text-sm leading-6 text-muted-foreground">
        原生输入、值状态、前后缀、附加内容与清空行为均由独立 primitive
        显式组合。
      </p>
    </header>
    ...
    <div class="grid gap-4">
      <Card
        ...
        ><InputGroup class="max-w-xl"
          ><InputGroupAddon>https://</InputGroupAddon><InputRoot
            ><InputControl
              aria-label="Domain"
        ...
        ><div class="grid gap-3 md:grid-cols-2">
          <div class="space-y-1.5">
            <InputRoot
              ...
              onValueChange={(next) => (controlledValue = next)}
              ><InputControl
                aria-label="受控输入"
                placeholder="受控输入"
              /></InputRoot
      ...
      <Card
        title="Prefix"
        description="InputPrefix 位于输入框内部、原生输入内容之前。"
        ><InputRoot class="max-w-md"
          ><InputPrefix aria-hidden="true">@</InputPrefix><InputControl
            aria-label="用户名"
      ...
      <Card
        title="Suffix"
        description="InputSuffix 位于输入框内部、原生输入内容之后。"
        ><InputRoot class="max-w-md"
          ><InputControl aria-label="域名" placeholder="example" /><InputSuffix
            >.com</InputSuffix
          ></InputRoot
        ></Card
      >
      <Card
        ...
        ><InputRoot class="max-w-xl"
          ><InputAddonBefore>https://</InputAddonBefore><InputControl
            aria-label="站点地址"
            placeholder="example"
          /><InputAddonAfter>/console</InputAddonAfter></InputRoot
        ></Card
      >
      <Card
        title="Clear"
        description="InputClear 仅在有值且可编辑时显示，清空后恢复输入焦点。"
        ><InputRoot defaultValue="clearable-value" class="max-w-md"
          ><InputControl aria-label="可清空输入" /><InputClear
            aria-label="清空输入"
          /></InputRoot
        ></Card
      >
      <Card
        ...
        ><div class="max-w-md space-y-2">
          <InputRoot
            value={manualValue}
            onValueChange={(next) => (manualValue = next)}
            ><InputControl
              bind:this={focusControl}
              aria-label="手动控制输入"
              placeholder="手动控制输入"
            /></InputRoot
          >
          <div class="flex flex-wrap gap-1.5">
            <Button size="sm" onclick={() => focusControl?.focus()}>聚焦</Button
            ><Button
              size="sm"
              variant="outline"
              onclick={() => focusControl?.blur()}>移除焦点</Button
            ><Button
              size="sm"
              variant="outline"
              onclick={() => (manualValue = "")}>手动清空</Button
            >
          </div>
        </div></Card
      >
