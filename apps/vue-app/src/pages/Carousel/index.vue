    ...
    <div class="mx-auto w-full max-w-5xl space-y-4">
      ...
      >
        <Carousel :options="{ loop: true }"
          ...
          >
            <div
              class="flex h-40 items-center justify-center rounded-md bg-primary text-lg font-semibold text-primary-foreground"
            >
              {{ slide }}
            </div>
          </div>
          <template #controls="{ controller, snapshot }"
            ><CarouselControls :controller="controller" :snapshot="snapshot" /></template
        ></Carousel>
      </Card>
      <Card
        title="Autoplay"
      ...
      >
        <Carousel :options="{ loop: true }" :autoplay="{ delay: 2500, stopOnInteraction: true }"
          ><div
            v-for="slide in slides"
            :key="slide"
            role="group"
      ...
      >
        <Carousel :plugins="[AutoHeight()]"
          ><article
            v-for="slide in autoHeightSlides"
            :key="slide.title"
            role="group"
            aria-roledescription="slide"
