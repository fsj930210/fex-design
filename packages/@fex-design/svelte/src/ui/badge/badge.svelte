...
{#if children && (dot || count !== undefined)}
  ...
  >
    <span data-slot="badge-content" class={classNames?.content} style={styles?.content}
      >{@render children()}</span
    >
    {#if dot}
      <Dot {color} {size} class={classNames?.indicator} style={indicatorStyle} />
    {:else}
      <PrimitiveBadge {...badgeProps} class={classNames?.indicator} />
    {/if}
  </span>
{:else if dot}
  <Dot {color} {size} class={cn(className, classNames?.indicator)} style={indicatorStyle} />
{:else}
  <PrimitiveBadge
    {...badgeProps}
