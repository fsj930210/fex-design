...
withDefaults(
  defineProps<{
    ...
    styles?: AnchorStyles<StyleValue>
  }>(),
  {
    defaultActiveKeys: () => [],
    activeMode: 'current',
...
const emit = defineEmits<{
  change: [
    ...
    items: readonly import('@fex-design/core/anchor/types').AnchorRegisteredItem[],
  ]
  itemClick: [event: MouseEvent, item: AnchorItem<string>]
}>()
...
<template>
  <AnchorRoot
    ...
    @change="(keys, entries) => emit('change', keys, entries)"
    ><AnchorRail :class="classNames.rail" :style="styles.rail"
      ><AnchorIndicator :class="classNames.indicator" :style="styles.indicator" /></AnchorRail
    ><AnchorItems
      :items="items"
      :class-names="classNames"
      :styles="styles"
      @item-click="(event, item) => emit('itemClick', event, item)"
  /></AnchorRoot>
</template>
