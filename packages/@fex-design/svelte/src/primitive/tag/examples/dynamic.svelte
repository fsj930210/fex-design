...
<div class="grid gap-3">
  <div class="flex flex-wrap gap-2">{#each tags as tag (tag.id)}<Tag color="primary">{tag.label}<TagAction aria-label={`删除${tag.label}`} onclick={() => removeTag(tag.id)} /></Tag>{/each}</div>
