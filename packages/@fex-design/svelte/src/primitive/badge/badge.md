...
## Import
    ...
    import { Badge, BadgeDot, BadgeGroup, BadgeRibbon } from '@fex-design/svelte/primitive/badge'
...
| content | children Snippet | — | Content used when `count` is absent. |
| native attributes | HTMLAttributes<HTMLSpanElement> | — | native element attributes. |
...
`BadgeGroup` accepts `maxCount` and `items`, required `item` Snippet, and optional `overflow` Snippet. Without `maxCount`, all items remain visible; the default overflow node is `+N`.

## BadgeRibbon API

`BadgeRibbon` accepts `color`, logical `placement="start" | "end"`, and children Snippet. Default placement is `end`; default color is `primary`.
