<script lang="ts">
  ...
  function hasToastIcon(item: { icon?: unknown | null; variant: string }) {
    return (
      item.icon !== null &&
      (item.icon !== undefined || isBuiltInIconVariant(item.variant))
    );
  }
  ...
  <div class="mx-auto w-full max-w-6xl space-y-3">
    <header class="space-y-2">
      ...
      <p class="max-w-2xl text-sm leading-6 text-muted-foreground">
        Primitive global feedback with quick calls, custom content, position,
        max count, manual destroy, stable id updates, and stacked display.
      </p>
    </header>
    ...
    >
      <div class="flex flex-wrap gap-1.5">
        <Button
          variant="outline"
          onclick={() => toast.show("Hello, Fex Design!")}>Default</Button
        >
        <Button
          variant="outline"
        ...
        <Button
          variant="outline"
          onclick={() => toast.info("This is an info message")}>Info</Button
        >
        <Button
          variant="outline"
        ...
        <Button
          variant="outline"
          onclick={() => toast.error("This is an error message")}>Error</Button
        >
        <Button
          variant="outline"
          onclick={() =>
            toast.loading({ title: "Loading data", duration: 2500 })}
          >Loading</Button
        >
      </div>
    </Card>
    ...
    >
      <div class="flex flex-wrap gap-1.5">
        ...
        <Button variant={stack ? "default" : "outline"} onclick={toggleStack}
          >{stack ? "Stack on" : "Stack off"}</Button
        >
      </div>
    </Card>
  </div>
