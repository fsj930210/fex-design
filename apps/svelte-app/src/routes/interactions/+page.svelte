<script lang="ts">
  ...
  function reportDrop(
    ...
    dropDemoItems = {
      ...
      "card-zone": [
        ...dropDemoItems["card-zone"].filter((id) => id !== itemId),
        ...(zoneId === "card-zone" ? [itemId] : []),
      ],
      "any-zone": [
        ...dropDemoItems["any-zone"].filter((id) => id !== itemId),
    ...
    use:draggableAction={{
      ...
      onOverlayStyleChange: (style) => {
        dragOverlay =
          style.display === "none"
            ? null
            : { label: item.label, style: style as Record<string, string> };
      },
    }}
    ...
    <header class="space-y-4">
      ...
      <div>
        ...
        <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Dropzone, movable, and 8-direction resize hooks for existing
          components.
        </p>
      </div>
    </header>
    ...
    <div class="space-y-4">
      ...
      >
        <div class="grid gap-2 md:grid-cols-[280px_1fr]">
          <div
            class="space-y-1.5 rounded-md border border-border bg-background p-2"
          >
          ...
          <div class="grid gap-2 md:grid-cols-2">
            <div
              use:droppableAction={{
                ...
                edges: ["top", "bottom"],
                onDragEnter: ({ source }) =>
                  (dropResult = `${String(source.id)} is over Cards only.`),
                onDragLeave: () =>
                  (dropResult = "Drop a draggable item into a zone."),
                onDrop: ({ source, edge }) =>
                  reportDrop("card-zone", "Cards only", source, edge),
              }}
            ...
            >
              <div>
                ...
                <div class="mt-2 space-y-1.5">
                  {#each dropDemoItems["card-zone"] as itemId (itemId)}{@render draggableToken(
                      itemById(itemId),
                    )}{/each}
                </div>
              </div>
            ...
            <div
              use:droppableAction={{
                ...
                edges: ["top", "bottom"],
                onDragEnter: ({ source }) =>
                  (dropResult = `${String(source.id)} is over Any item.`),
                onDragLeave: () =>
                  (dropResult = "Drop a draggable item into a zone."),
                onDrop: ({ source, edge }) =>
                  reportDrop("any-zone", "Any item", source, edge),
              }}
              class="flex min-h-36 flex-col justify-between rounded-md border border-dashed border-border bg-background p-2 text-sm transition-colors data-[can-drop=true]:border-focus data-[over=true]:bg-selected-background"
            >
              <div>
                ...
                <p class="mt-1.5 text-muted-foreground">Drop target</p>
                <div class="mt-2 space-y-1.5">
                  {#each dropDemoItems["any-zone"] as itemId (itemId)}{@render draggableToken(
                      itemById(itemId),
                    )}{/each}
                </div>
              </div>
      ...
      <Card
        title="Dropzone"
        description="The action handles drag state, validation, and hidden input selection."
      >
        <div
          use:dropzoneAction={{
            accept: ["image/*", ".txt"],
        ...
        >
          ...
          <span
            >{files.length > 0
              ? files.join(", ")
              : "Drop images or text files here"}</span
          >
        </div>
      </Card>
      ...
      <Card
        title="Move"
        description="useMove adds draggable behavior to any existing element."
      >
        <div
          class="relative h-64 overflow-hidden rounded-md border border-border bg-background"
        >
          <div
          ...
          >
            ...
            >
              <span>Drag this title bar</span><span
                class="text-muted-foreground">::</span
              >
            </div>
            <p class="p-2 text-sm text-muted-foreground">
              The card is constrained to its parent. The title bar is the move
              handle.
            </p>
          </div>
        </div>
      ...
      >
        <div
          class="relative h-80 overflow-hidden rounded-md border border-border bg-background"
        >
          <div
            use:resizeAction={{
          ...
          >
            ...
            <p class="mt-1.5 text-sm text-muted-foreground">
              Drag any edge or corner. Handles are intentionally visible for
              this demo.
            </p>
            <div
