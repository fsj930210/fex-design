...
Native contract: Svelte `HTMLInputAttributes`. `bind:ref` exposes the native `HTMLInputElement`.
...
`class`, `classNames`, and native style strings. State-specific colors use selectors such as `hover`, `focus-within`, data attributes, and `[aria-invalid=true]`; CSS variables stay limited to reusable base tokens.
