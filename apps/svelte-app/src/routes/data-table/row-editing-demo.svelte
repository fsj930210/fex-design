<script lang="ts">
  ...
  const features: Features = tableFeatures({
    columnSizingFeature,
    columnMeta: {},
  });
  let rows = $state(people6),
    draft = $state<Person | null>(null);
  ...
  function updateField(
    fieldName: "name" | "department" | "status",
    value: string,
  ) {
    if (draft) draft = { ...draft, [fieldName]: value };
  }
  function save() {
    if (!draft) return;
    rows = rows.map((row) => (row.id === draft!.id ? draft! : row));
    draft = null;
...
{#snippet cell(
  item: Cell<Features, Person>,
)}{#if item.column.id === "__actions__"}{#if draft?.id === item.row.id}<span
        class="inline-flex gap-1.5"
        ><Button size="sm" onclick={save}>Save</Button><Button
          size="sm"
          variant="outline"
      ...
      >{:else}<Button
        size="sm"
        variant="outline"
        onclick={() => (draft = { ...item.row.original })}>Edit row</Button
      >{/if}{:else if draft?.id === item.row.id}<InputRoot
      value={String(
        draft?.[item.column.id as "name" | "department" | "status"] ?? "",
      )}
      ><InputControl
        oninput={(event) =>
          updateField(
            item.column.id as "name" | "department" | "status",
            event.currentTarget.value,
          )}
      /></InputRoot
    >{:else}{String(item.getValue() ?? "")}{/if}{/snippet}
<DemoSection
  title="Row editing"
