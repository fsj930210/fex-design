...
| 内容     | `children?: JSX.Element`                    | XIcon  | 传入内容时完全替换默认关闭图标。                 |
...
内容遵循 Solid 原生模型：`children?: JSX.Element`。需要标签内操作时，在 `Tag` 内组合 `TagAction`。它默认显示 XIcon，自定义内容可以表达删除、编辑、更多或其他操作；`TagAction` 不规定具体行为，也不会自动联动父级 disabled。
