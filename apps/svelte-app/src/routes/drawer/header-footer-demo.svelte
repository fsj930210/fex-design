<script lang="ts">
  ...
  import DrawerFooter from "@fex-design/svelte/primitive/drawer-footer";
...
<Drawer
  ><DrawerTrigger
    >{#snippet children(props)}<Button {...props} variant="outline"
        >Header + footer</Button
      >{/snippet}</DrawerTrigger
  ><DrawerPortal
    ><DrawerMask /><DrawerContent aria-label="Header footer drawer"
      ><DrawerHeader>Custom header<DrawerClose /></DrawerHeader><DrawerBody
        >Body remains the scrollable region.</DrawerBody
      ><DrawerFooter
        ><DrawerClose
          class="!size-auto rounded-md bg-primary px-3 py-2 text-base leading-6 text-primary-foreground hover:bg-primary/90"
          >Done</DrawerClose
        ></DrawerFooter
      ></DrawerContent
    ></DrawerPortal
