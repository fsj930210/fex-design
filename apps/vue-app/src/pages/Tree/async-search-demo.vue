...
const keyword = ref(''),
  results = ref<DemoTreeSearchResult[]>([]),
  treeData = ref<DepartmentNode[]>([]),
  selected = ref('')
...
async function choose(result: DemoTreeSearchResult) {
  selected.value = result.node.id
  keyword.value = ''
  treeData.value = convert((await getDemoTreeSubtree(result.node.id)).treeData)
}
function clear() {
  keyword.value = ''
  results.value = []
...
<template>
  <Card
    ...
    description="The real server returns path-aware matches, then an ancestor subtree."
  >
    <InputRoot
      :value="keyword || results.find((item) => item.node.id === selected)?.node.name || selected"
      class="mb-2 max-w-sm"
      @value-change="search"
      @clear="clear"
