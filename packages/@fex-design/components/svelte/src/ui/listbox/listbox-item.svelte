<script lang="ts">
  ...
  import PrimitiveListboxItem from '@fex-design/svelte/primitive/listbox/listbox-item.svelte';
  import PrimitiveListboxItemIndicator from '@fex-design/svelte/primitive/listbox/listbox-item-indicator.svelte';
...
<PrimitiveListboxItem
  {...rest}
  {value}
  class={cn(listboxItemClassName({}), className)}
>
  {#if children}
    {@render children()}
  {:else}
    <span class={listboxItemContentClassName}>
      <span class={listboxItemTitleClassName}>{title}</span>
      {#if description}
        <span class={listboxItemDescriptionClassName}>{description}</span>
      {/if}
    </span>
