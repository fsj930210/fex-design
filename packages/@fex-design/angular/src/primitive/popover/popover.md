...
`createPopover(options)` is the public Angular adapter for custom Popover markup. It owns a stable Core controller and returns controller and snapshot signal. Use controller methods to open/close, connect reference/content/arrow elements, and subscribe to state. Internal context readers are not part of this standalone API.
