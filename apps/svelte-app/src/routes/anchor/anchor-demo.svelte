<script lang="ts">
  ...
  const items: AnchorItem<string>[] = [
    { key: "anchor-overview", title: "Overview", target: "#anchor-overview" },
    {
      ...
      target: "#anchor-api",
      children: [
        { key: "anchor-props", title: "Props", target: "#anchor-props" },
      ],
    },
...
>
  <div class="grid gap-3 lg:grid-cols-[10rem_10rem_1fr]">
    <div>
      ...
      <Anchor
        ...
        onchange={(keys) => (currentKeys = keys)}
      />
    </div>
    <div>
      ...
      <Anchor
        ...
        targetOffset={80}
        container={() => container ?? window}
        onchange={(keys) => (activeKeys = keys)}
      />
    </div>
    <div
      bind:this={container}
      class="h-72 overflow-y-auto rounded-md border border-border p-3"
    >
  ...
  <p class="mt-2 text-xs text-muted-foreground">
    current: {currentKeys.join(", ") || "none"} · progress: {activeKeys.join(
      ", ",
    ) || "none"}
  </p>
</Card>
