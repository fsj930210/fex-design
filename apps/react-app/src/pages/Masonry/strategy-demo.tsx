...
function StrategyExample({ placement }: { placement: MasonryPlacement }) {
  ...
  const handleLayout = (layout: MasonryLayoutDetail) =>
    setColumns(Object.fromEntries(layout.items.map((item) => [String(item.key), item.column + 1])))
  return (
    <section className="grid gap-1.5">
      <div>
        <h3 className="font-medium">
          {placement === 'ordered' ? '按输入顺序（ordered）' : '按当前最短列（shortest）'}
        </h3>
      ...
      <MasonryRoot columns={3} gap={12} placement={placement} onLayoutChange={handleLayout}>
        <MasonryViewport>
...
function PinnedExample() {
  return (
    <section className="grid gap-1.5">
      <div>
        <h3 className="font-medium">指定列与自动分配混排</h3>
        <p className="text-sm text-muted-foreground">
          Card 2 固定第 3 列；其他项目仍按最短列分配。列数不足时固定列会自动收敛到有效范围。
        </p>
      </div>
      <MasonryRoot columns={{ minColumnWidth: 160, max: 3 }} gap={{ column: 12, row: 20 }}>
        <MasonryViewport>
...
export function StrategyMasonryDemo() {
  return (
    <div className="grid gap-4">
      <StrategyExample placement="ordered" />
      <StrategyExample placement="shortest" />
      <PinnedExample />
    </div>
  )
}
