...
const multipleValue = ref([15, 45, 75])
const disabledValue = ref([60])

function setBasicValue(next: number[]) {
  basicValue.value = next
}

function setControlledValue(next: number[]) {
  controlledValue.value = next
}

function setRangeValue(next: number[]) {
  rangeValue.value = next
}

function setMultipleValue(next: number[]) {
  multipleValue.value = next
}
</script>
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex items-center gap-4">
            ...
            <SliderRoot orientation="vertical" :default-value="[60]" aria-label="Vertical slider"
              ><SliderTrack><SliderRange /></SliderTrack><SliderThumb aria-label="Vertical slider"
            /></SliderRoot>
          </div>
        </Card>
        <Card title="Sizes" description="Slider supports sm, default, and lg sizes.">
          <div class="grid gap-2">
            ...
            /></SliderRoot>
            <SliderRoot :default-value="[50]" aria-label="Default slider"
              ><SliderTrack><SliderRange /></SliderTrack><SliderThumb aria-label="Default slider"
            /></SliderRoot>
            <SliderRoot size="lg" :default-value="[75]" aria-label="Large slider"
              ><SliderTrack><SliderRange /></SliderTrack><SliderThumb aria-label="Large slider"
            /></SliderRoot>
          </div>
        </Card>
        <Card title="Disabled" description="Disabled state blocks input.">
          <div class="grid gap-1.5">
            ...
            <p class="text-sm text-muted-foreground">
              Current value: {{ disabledValue.join(', ') }}
            </p>
          </div>
        </Card>
      </div>
