...
<Demo
  ...
  >
    <FieldRoot required {invalid} hasError={invalid}
      ><FieldLabel>所在地区</FieldLabel><FieldControl
        >{#snippet children(binding)}<DemoCascader
            options={regionOptions}
            value={value as string[]}
            status={invalid ? "error" : undefined}
            triggerProps={binding.props}
            onChange={(next) => {
      ...
      >{#if invalid}<FieldError
          >{#snippet children()}请选择所在地区{/snippet}</FieldError
        >{/if}</FieldRoot
