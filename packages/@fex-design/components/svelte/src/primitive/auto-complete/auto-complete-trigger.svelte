<script lang="ts">
  ...
  function keydown(event: KeyboardEvent) {
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      autoComplete.controller.setOpen(false, "escape");
  }
  ...
  {#snippet children(trigger)}
    ...
    >
      ...
      <InputControl
        ...
        aria-activedescendant={$snapshot.activeKey === undefined
          ? undefined
          : `${autoComplete.listId}-${$snapshot.activeKey}`}
        onfocus={(event) => {
          onfocus?.(event);
          if (!event.defaultPrevented)
            autoComplete.controller.setOpen(true, "focus");
        }}
        onblur={(event) => {
          onblur?.(event);
          if (!event.defaultPrevented)
            autoComplete.controller.setOpen(false, "blur");
        }}
        onkeydown={keydown}
      />
      ...
      {#if autoComplete.loading() || suffix}<InputSuffix
          >{#if autoComplete.loading()}<LoadingIcon
              class="animate-spin"
            />{:else}{@render suffix?.()}{/if}</InputSuffix
        >{/if}
    </InputRoot>
  {/snippet}
</PopoverTrigger>
