<script lang="ts">
  ...
  import { TreeRoot, TreeViewport } from "@fex-design/svelte/primitive/tree";
  import Card from "@fex-design/svelte/ui/card";
  import DndTreeRow from "./dnd-tree-row.svelte";
  import {
...
<Card
  ...
  ><TreeRoot
    ...
    >{#snippet children()}<TreeViewport
        >{#snippet children(item)}<DndTreeRow
            {controller}
            itemKey={item.key}
