...
>
  ...
  import type { SvelteDataTable } from '@fex-design/svelte/stores/create-data-table';
  ...
  const pagination = $derived(
    ($snapshot.state as { pagination: { pageIndex: number; pageSize: number } })
      .pagination,
  );
  ...
  const rowCount = $derived.by(() => {
    void $snapshot;
    return paginationTable.getRowCount();
  });
  const pageCount = $derived.by(() => {
    void $snapshot;
    return Math.max(1, paginationTable.getPageCount());
  });
  const canPrevious = $derived.by(() => {
    void $snapshot;
    return paginationTable.getCanPreviousPage();
  });
  const canNext = $derived.by(() => {
    void $snapshot;
    return paginationTable.getCanNextPage();
  });
</script>
  ...
  <div class={dataTableControlsClassName}>
    <label>
      ...
      <select
        ...
        onchange={(event) =>
          paginationTable.setPageSize(Number(event.currentTarget.value))}
      >
        {#each pageSizeOptions as size (size)}<option value={size}
            >{size} / page</option
          >{/each}
      </select>
    </label>
    ...
    <Button
      ...
      onclick={() => paginationTable.previousPage()}>Previous</Button
    >
    <Button
      size="sm"
      variant="outlined"
      disabled={!canNext}
      onclick={() => paginationTable.nextPage()}>Next</Button
    >
  </div>
</div>
