<script lang="ts">
  ...
  interface UploadTriggerBindings {
    type: "button";
    disabled: boolean;
  ...
  let { children }: { children?: Snippet<[{ props: UploadTriggerBindings }]> } =
    $props();
...
{#if children}{@render children({ props: triggerProps })}{:else}<PrimitiveButton
    {...triggerProps}>选择文件</PrimitiveButton
  >{/if}
