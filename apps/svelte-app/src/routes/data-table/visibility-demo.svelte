...
<DemoSection
  ...
  ><section class="space-y-1.5">
    <h3 class="text-sm font-medium text-foreground">
      Toggle visible leaf columns
    </h3>
    <div class="flex flex-wrap gap-1.5 rounded-md border border-border p-1.5">
      {#each table
        .getAllLeafColumns()
        .filter((column) => column.getCanHide()) as column (column.id)}<label
