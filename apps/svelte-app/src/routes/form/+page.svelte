<script lang="ts">
  ...
  import FieldGroup from "@fex-design/svelte/primitive/field-group";
  import FieldSet from "@fex-design/svelte/primitive/field-set";
  import FieldLegend from "@fex-design/svelte/primitive/field-legend";
  import FieldContent from "@fex-design/svelte/primitive/field-content";
  import FieldTitle from "@fex-design/svelte/primitive/field-title";
  import FormHost from "@fex-design/svelte/primitive/form";
  import { scrollToField } from "@fex-design/core";
  ...
  let instanceResult = $state("点击按钮读取或设置字段值");
  ...
  const contactsForm = createForm(() => ({
    defaultValues: {
      contacts: [
        { id: "primary", name: "", email: "", notifications: true },
      ] as Contact[],
    },
    onSubmit: ({ value }) => {
      result = `联系人：${JSON.stringify(value)}`;
    },
  }));
  const prefixedForm = createForm(() => ({
    defaultValues: { profile: { displayName: "", email: "" } },
    onSubmit: ({ value }) => {
      result = `路径前缀：${JSON.stringify(value)}`;
    },
  }));
  const profileForm = createForm(() => ({
    defaultValues: { name: "", email: "" },
    onSubmit: ({ value }) => {
      result = `资料表单：${JSON.stringify(value)}`;
    },
  }));
  const securityForm = createForm(() => ({
    defaultValues: { notificationEmail: "", twoFactor: false },
    onSubmit: ({ value }) => {
      result = `安全表单：${JSON.stringify(value)}`;
    },
  }));
  const scrollForm = createForm(() => ({
    defaultValues: { note: "", deliveryAddress: "" },
    onSubmit: ({ value }) => {
      result = `长表单：${JSON.stringify(value)}`;
    },
  }));
  const accountValidators = {
    onBlur: ({ value }: { value: string }) =>
      value.trim() ? undefined : "请输入账号",
  ...
  const newPasswordValidators = {
    onChange: ({ value }: { value: string }) =>
      value.length >= 8 ? undefined : "密码至少 8 位",
  };
  ...
  const nicknameValidators = {
    onChange: ({ value }: { value: string }) =>
      dynamicRuleForm.getFieldValue("nicknameRequired") && !value.trim()
        ? "请输入昵称"
        : undefined,
  ...
  const displayNameValidators = {
    onChange: ({ value }: { value: string }) =>
      value.trim() ? undefined : "请输入显示名称",
  };
  const deliveryAddressValidators = {
    onSubmit: ({ value }: { value: string }) =>
      value.trim() ? undefined : "请输入收货地址",
  };
</script>
  ...
  <div class="mx-auto w-full max-w-5xl space-y-4">
    <header class="space-y-2">
      ...
      <div>
        ...
        <p class="max-w-3xl text-sm leading-6 text-muted-foreground">
          ...
          是唯一字段状态入口。FieldRoot、Label、Control、Description 和 Error
          只负责 DOM 语义、可访问性与结构。
        </p>
      </div>
    </header>
    <Card
      ...
      ><FormHost form={loginForm} class="grid max-w-xl gap-3"
        ><Field name="account" validators={accountValidators}
          >{#snippet children(field)}{@const invalid =
              field.state.meta.isTouched && !field.state.meta.isValid}<FieldRoot
              required
              {invalid}
              hasDescription
              hasError={invalid}
              ...
              ><FieldControl
                >{#snippet children(binding)}<InputRoot
              ...
              ><FieldDescription
                >使用 validators.onBlur 校验必填。</FieldDescription
              >{#if invalid}<FieldError
                  >{#snippet children()}{#each errors(field.state.meta.errors) as error}<div
                      >
                        {error}
                      </div>{/each}{/snippet}</FieldError
                >{/if}</FieldRoot
            >{/snippet}</Field
        ><Field name="password" validators={loginPasswordValidators}
          >{#snippet children(field)}{@const invalid =
              field.state.meta.isTouched && !field.state.meta.isValid}<FieldRoot
              required
              {invalid}
            ...
            >{/snippet}</Field
        ><Field name="remember"
          >{#snippet children(field)}<FieldRoot orientation="horizontal"
              ><FieldControl
                >{#snippet children(binding)}<Checkbox
                    {...binding.props}
                  ...
                  />{/snippet}</FieldControl
              ><FieldContent
                ><FieldLabel>记住登录状态</FieldLabel><FieldDescription
                  >自定义控件显式绑定值与事件。</FieldDescription
                ></FieldContent
              ></FieldRoot
            >{/snippet}</Field
        >
        <div class="flex gap-1.5">
          <loginForm.Subscribe
            ...
            >{#snippet children(state)}<Button
                type="submit"
                disabled={!state[0] || state[1]}
                >{state[1] ? "提交中…" : "登录"}</Button
              >{/snippet}</loginForm.Subscribe
          ><Button
            type="button"
            variant="outline"
            onclick={() => loginForm.reset()}>重置</Button
          >
        </div></FormHost
      ></Card
    >
    <Card
      ...
      ><FormHost form={validationForm} class="grid max-w-xl gap-3"
        ><Field name="username" validators={usernameValidators}
          >{#snippet children(field)}{@const invalid =
              field.state.meta.isTouched && !field.state.meta.isValid}<FieldRoot
              required
              {invalid}
              ...
              >{#if invalid}<FieldError
                ...
                >{/if}</FieldRoot
            >{/snippet}</Field
        ><Field name="password" validators={newPasswordValidators}
          >{#snippet children(field)}{@const invalid =
              field.state.meta.isTouched && !field.state.meta.isValid}<FieldRoot
              required
              {invalid}
              ...
              >{#if invalid}<FieldError
                ...
                >{/if}</FieldRoot
            >{/snippet}</Field
        ><Field name="confirmPassword" validators={confirmValidators}
          >{#snippet children(field)}{@const invalid =
              field.state.meta.isTouched && !field.state.meta.isValid}<FieldRoot
              required
              {invalid}
    ...
    <Card
      ...
      ><div class="grid gap-4 lg:grid-cols-2">
            ...
            >{#snippet children(field)}<FieldRoot orientation="horizontal"
                ><FieldControl
                  >{#snippet children(binding)}<Checkbox
                    ...
                    />{/snippet}</FieldControl
                ><FieldContent
                  ><FieldLabel>昵称必填</FieldLabel><FieldDescription
                    >开关改变后，昵称字段的 onChange 校验会自动重跑。</FieldDescription
                  ></FieldContent
                ></FieldRoot
              >{/snippet}</Field
          ><Field name="nickname" validators={nicknameValidators}
            >{#snippet children(field)}{@const invalid =
                field.state.meta.isTouched &&
                !field.state.meta.isValid}<FieldRoot
                required={dynamicRuleForm.getFieldValue("nicknameRequired")}
                {invalid}
                hasError={invalid}
                ><FieldLabel>昵称</FieldLabel><FieldControl
                  >{#snippet children(binding)}<InputRoot
                      value={field.state.value}
                      onValueChange={field.handleChange}
                      {invalid}
        ...
        ><FormHost form={cascadeForm} class="grid gap-3"
          ><Field name="province"
            >{#snippet children(field)}<FieldRoot
                ><FieldLabel>省份</FieldLabel><FieldControl
                  >{#snippet children(binding)}<select
                      {...binding.props}
                      class={selectClass}
                      value={field.state.value}
                      onchange={(event) => {
                        ...
                        cascadeForm.setFieldValue(
                          "city",
                          locations[province][0],
                        );
                      }}
                      >{#each Object.keys(locations) as province}<option
          ...
          ><cascadeForm.Subscribe selector={(state) => state.values.province}
            >{#snippet children(province)}<Field name="city"
                >{#snippet children(field)}<FieldRoot hasDescription
                    ><FieldLabel>城市</FieldLabel><FieldControl
                      >{#snippet children(binding)}<select
                          {...binding.props}
                          class={selectClass}
                          value={field.state.value}
                          ...
                          >{#each locations[province] as city}<option
                              value={city}>{city}</option
                            >{/each}</select
                        >{/snippet}</FieldControl
                    ><FieldDescription
                      >省份 change 事件中同步重设城市。</FieldDescription
                    ></FieldRoot
                  >{/snippet}</Field
              >{/snippet}</cascadeForm.Subscribe
    ...
    <Card
      ...
      ><FieldGroup
        ><FieldRoot orientation="vertical"
          ><FieldLabel>Vertical</FieldLabel><InputRoot value=""
            ><InputControl placeholder="标签在上方" /></InputRoot
          ></FieldRoot
        ><FieldRoot orientation="horizontal"
          ><FieldLabel>Horizontal</FieldLabel><InputRoot value=""
            ><InputControl placeholder="标签按内容宽度排列" /></InputRoot
          ></FieldRoot
        ><FieldRoot orientation="responsive"
          ><FieldLabel>Responsive</FieldLabel><InputRoot value=""
            ><InputControl placeholder="窄屏纵向，变宽后横向" /></InputRoot
          ></FieldRoot
        ></FieldGroup
      ><FieldGroup orientation="inline" class="mt-3"
        ><FieldRoot orientation="inline"
          ><FieldLabel>关键字</FieldLabel><InputRoot value=""
            ><InputControl class="w-48" placeholder="内联筛选" /></InputRoot
          ></FieldRoot
        ><FieldRoot orientation="inline"
          ><FieldLabel>状态</FieldLabel><select class={`${selectClass} w-32`}
            ><option>全部</option><option>启用</option></select
          ></FieldRoot
    ...
    <Card
      ...
      ><FormHost form={instanceForm} class="grid max-w-xl gap-3"
        ><Field name="source"
          >{#snippet children(field)}<FieldRoot hasDescription
              ><FieldLabel>Form 初始值</FieldLabel><FieldControl
                >{#snippet children(binding)}<InputRoot
                    value={field.state.value}
                    onValueChange={field.handleChange}
                    ><InputControl
        ...
        ><Field name="nickname" defaultValue="来自 Field defaultValue"
          >{#snippet children(field)}<FieldRoot hasDescription
              ><FieldLabel>字段初始值</FieldLabel><FieldControl
                >{#snippet children(binding)}<InputRoot
                    value={field.state.value}
                    onValueChange={field.handleChange}
                    ><InputControl
                  ...
                  >{/snippet}</FieldControl
              ><FieldDescription
                >使用 TanStack 原生 defaultValue。</FieldDescription
              ></FieldRoot
            >{/snippet}</Field
        >
        <div class="flex flex-wrap gap-1.5">
          <Button
            type="button"
            onclick={() =>
              instanceForm.setFieldValue("nickname", "通过 setFieldValue 设置")}
            >设置昵称</Button
          ><Button
            type="button"
            variant="outline"
            onclick={() => {
              instanceResult = String(instanceForm.getFieldValue("nickname"));
            }}>读取昵称</Button
          ><Button
            type="button"
            variant="outline"
            onclick={() => instanceForm.reset()}>重置</Button
          >
        </div>
        <p
    ...
    <Card
      ...
      ><FormHost form={contactsForm} class="grid gap-3"
        ><Field
          ...
          >{#snippet children(contactsField)}<FieldSet
              ><FieldLegend>联系人</FieldLegend>
              <p class="text-sm text-muted-foreground">
                每位联系人的姓名、邮箱和通知偏好都是独立的嵌套字段。
              </p>
              <FieldGroup
                >{#each contactsField.state.value as contact, index (contact.id)}<div
                    class="rounded-md border border-border p-4"
                  >
                    <FieldTitle>联系人 {index + 1}</FieldTitle><FieldGroup
                      class="mt-2"
                      ><Field
                        ...
                        >{#snippet children(field)}{@const invalid =
                            ...
                            hasError={invalid}
                            ><FieldLabel
                              >姓名 <FieldRequiredIndicator /></FieldLabel
                            ><FieldControl
                              >{#snippet children(binding)}<InputRoot
                                  value={field.state.value}
                                  onValueChange={field.handleChange}
                      ...
                      ><Field
                        ...
                        >{#snippet children(field)}{@const invalid =
                            ...
                            hasError={invalid}
                            ><FieldLabel
                              >邮箱 <FieldRequiredIndicator /></FieldLabel
                            ><FieldControl
                              >{#snippet children(binding)}<InputRoot
                                  value={field.state.value}
                                  onValueChange={field.handleChange}
                    ...
                    <div class="mt-2 flex gap-1.5">
                      <Button
                        ...
                        variant="outline"
                        onclick={() =>
                          void contactsField.insertValue(
                            index + 1,
                            createContact(),
                          )}>在后面插入</Button
                      ><Button
    ...
    <Card
      ...
      ><div class="grid gap-4 lg:grid-cols-2">
        <FormHost form={prefixedForm} class="grid gap-3"
          ><FieldSet
            ><FieldLegend>profile</FieldLegend><FieldGroup
              ><Field
              ...
              ><Field
                ...
                >{#snippet children(field)}{@const invalid =
                    ...
                    {invalid}
                    hasError={invalid}
                    ><FieldLabel>联系邮箱</FieldLabel><FieldControl
                      >{#snippet children(binding)}<InputRoot
                          value={field.state.value}
                          onValueChange={field.handleChange}
                          {invalid}
        ...
        <div class="grid gap-2">
          <FormHost
            ...
            ><FieldTitle>资料表单</FieldTitle><Field name="name"
              >{#snippet children(field)}<FieldRoot
                  ><FieldLabel>姓名</FieldLabel><FieldControl
                    >{#snippet children(binding)}<InputRoot
                        value={field.state.value}
                        onValueChange={field.handleChange}
                        ><InputControl
                  ...
                  ></FieldRoot
                >{/snippet}</Field
            ><Field name="email" validators={{ onChange: emailValidator }}
              >{#snippet children(field)}{@const invalid =
                  field.state.meta.isTouched &&
                  !field.state.meta.isValid}<FieldRoot
                  required
          ...
          ><FormHost
            form={securityForm}
            class="grid gap-2 rounded-md border border-border p-4"
            ><FieldTitle>安全表单</FieldTitle><Field
    ...
    <Card
      ...
      ><FormHost form={scrollForm} class="grid max-w-xl gap-3"
        ><div class="flex flex-wrap gap-1.5">
          <Button
            type="button"
            variant="outline"
        ...
        <Field name="note"
          >{#snippet children(field)}<FieldRoot
              ><FieldLabel>备注（选填）</FieldLabel><FieldControl
                >{#snippet children(binding)}<InputRoot
                    value={field.state.value}
                    onValueChange={field.handleChange}
                    ><InputControl
        ...
        <Field name="deliveryAddress" validators={deliveryAddressValidators}
          >{#snippet children(field)}{@const invalid =
              field.state.meta.isTouched && !field.state.meta.isValid}<FieldRoot
              required
              {invalid}
    ...
    <Card title="Latest result"
      ><pre
        class="overflow-auto rounded-md bg-muted-background p-3 text-sm text-muted-foreground">{result}</pre></Card
    >
  </div>
</main>
