...
<Card
  ...
  ><Form {form} class="space-y-3"
    ><Field
      ...
      >{#snippet children(field)}{@const errors =
          field.state.meta.errors.map(String)}{@const invalid =
          errors.length > 0}<FieldRoot {invalid} hasError={invalid}
          ><FieldLabel>Project members</FieldLabel><FieldControl
            >{#snippet children(binding)}<Transfer
                {...binding.props}
                items={members}
                {fieldNames}
        ...
        >{/snippet}</Field
    ><Button type="submit"
      >{#snippet children()}Validate assignment{/snippet}</Button
    ></Form
  ></Card
>
