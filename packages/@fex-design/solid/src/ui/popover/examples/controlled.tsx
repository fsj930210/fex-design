...
const cases: DemoCase[] = [
  { label: '受控表单', options: { get open() { return open() }, onOpenChange: setOpen } },
      ...
      <For each={cases}>{(item) =>
        <div>
          <Popover {...item.options} title={item.label}
            content={(state) => <div class="grid gap-3"><label>备注<input aria-label="备注" class="block rounded border p-2" placeholder="关闭后再打开检查草稿" /></label><Button onClick={() => state.close()}>在浮层内关闭</Button></div>}>
