  ...
  function update(id: string, field: Field, value: string) {
    rows = rows.map((row) =>
      row.id === id
        ? ({
            ...row,
            [field]: field === "visits" ? Number(value) || 0 : value,
          } as Person)
        : row,
    );
    editing = null;
...
{#snippet cell(
  item: Cell<Features, Person>,
)}{#if editing?.rowId === item.row.id && editing?.field === item.column.id}<InputRoot
    ...
    >{:else}<Button
      ...
      onclick={() =>
        (editing = { rowId: item.row.id, field: item.column.id as Field })}
      >{String(item.getValue() ?? "")}</Button
    >{/if}{/snippet}
<DemoSection
  title="Cell editing"
  ...
  ><section class="space-y-1.5">
    <h3 class="text-sm font-medium text-foreground">
      Editable name, status and visits
    </h3>
