<script lang="ts">
  ...
  const tricks = [
    ...
    },
    {
      value: "tre-flip",
      title: "Tre Flip",
      description: "A 360 flip with a backside shove-it",
    },
    {
      value: "fs-540",
      title: "FS 540",
...
<main class="min-h-screen bg-secondary-background px-2 md:px-6 py-4">
  <div class="mx-auto w-full max-w-5xl space-y-4">
    <header class="space-y-2">
      ...
      <div>
        ...
        <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Selection list primitives with single, multiple, vertical, horizontal
          and grouped layouts.
        </p>
      </div>
    </header>
    ...
    <div class="space-y-4">
      ...
      <Card
        title="Single"
        description="Controlled single selection uses onChange."
      >
        <Listbox
          items={tricks}
          value={selectedTrick}
      ...
      <Card title="Multiple" description="Multiple selection returns an array.">
        <Listbox
          ...
          onChange={(
            ...
            (selectedTricks = Array.isArray(values)
              ? values
              : values === undefined
                ? []
                : [values])}
      ...
      <Card
        title="Horizontal"
        description="Horizontal orientation lays items out in rows."
      >
        <Listbox
          items={tricks}
          defaultValue="tre-flip"
      ...
      <Card
        title="Grouped Items"
        description="Group related options with the same controller."
      >
        <Listbox
          multiple
          items={tricks}
