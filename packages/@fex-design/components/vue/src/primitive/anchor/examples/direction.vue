...
<template>
  <div class="grid w-full gap-6 sm:grid-cols-2">
    <section
      ...
      :key="direction"
      :ref="(element) => (containers[direction] = element as HTMLElement)"
      :dir="direction"
      ...
      <div class="grid min-h-full grid-cols-[8rem_minmax(0,1fr)] gap-6 p-4">
        <div class="sticky top-4 self-start">
          ...
          <AnchorRoot :container="() => containers[direction]" class="mt-3">
            ...
            <AnchorList>
              <AnchorItem :value="`${direction}-overview`" :target="`#vue-${direction}-overview`"
                ><AnchorLink>{{
                  direction === 'rtl' ? 'نظرة عامة' : '概览'
                }}</AnchorLink></AnchorItem
              >
              <AnchorItem :value="`${direction}-usage`" :target="`#vue-${direction}-usage`">
                <AnchorLink>{{ direction === 'rtl' ? 'طريقة الاستخدام' : '使用方式' }}</AnchorLink>
                <AnchorList
                  ><AnchorItem :value="`${direction}-api`" :target="`#vue-${direction}-api`"
                    ><AnchorLink>{{
        ...
        <div class="grid gap-32 pb-64">
          <section :id="`vue-${direction}-overview`">
            {{ direction === 'rtl' ? 'نظرة عامة' : '概览' }}
          </section>
          <section :id="`vue-${direction}-usage`">
            {{ direction === 'rtl' ? 'طريقة الاستخدام' : '使用方式' }}
          </section>
          <section :id="`vue-${direction}-api`">
            {{ direction === 'rtl' ? 'واجهة البرمجة' : 'API' }}
          </section>
        </div>
      </div>
