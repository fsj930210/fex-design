<script lang="ts">
  ...
  const clientFeatures: ClientFeatures = tableFeatures({
    ...clientModules,
    columnMeta: {},
  });
  ...
  const serverFeatures: ServerFeatures = tableFeatures({
    ...serverModules,
    columnMeta: {},
  });
    ...
    <section class="space-y-1.5">
      ...
      <div>
        ...
        <p class="px-1.5 pb-1.5 text-xs text-muted-foreground">
          Request: page={serverPagination.pageIndex +
            1}&amp;pageSize={serverPagination.pageSize}
        </p>
      </div>
    </section>
  </div>
