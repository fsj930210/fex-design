<script lang="ts">
  ...
  const features = $derived([
    ...
    ...(multiple
      ? [checkFeature({ mode: checkStrictly ? "strict" : "cascade" })]
      : [selectionFeature()]),
  ]);
</script>
...
<TreeSelectRoot
  ...
  onSearchValueChange={(next) => onSearch?.(next)}
>
    ...
    <TreeSelectTrigger>
      {#snippet children(state)}
        <div use:state.trigger.action {...state.trigger.props} class="w-80">
          ...
          >
            {#if multiple}{#if state.selectedItems.length <= maxTagCount}{#each state.selectedItems as item (item.value)}<Tag
                    size="sm"
                    closable
                    class="ml-1.5"
                    ...
                    onClose={(event) => {
                      ...
                      root.controller.setValues(
                        root.state.values.filter(
                          (value) => value !== item.value,
                        ),
                      );
                    }}>{item.label}</Tag
                  >{/each}{:else}<Tag size="sm" class="ml-1.5"
                  >已选择 {state.selectedItems.length} 项</Tag
                >{/if}{/if}
            <InputControl
              readonly={state.inputProps.readonly}
              placeholder={state.selectedItems.length
                ? undefined
                : searchable
                  ? "搜索部门"
                  : "请选择部门"}
            ...
            />
            <InputClear
              forceMount={state.selectedItems.length > 0 ||
                Boolean(searchValue)}
              aria-label="清除"
              onclick={(event) => {
                event.stopPropagation();
      ...
      {:else}
        ...
        >
          {#snippet title({ item })}
            ...
            >
              {#snippet children(option)}<button
                  type="button"
                  disabled={item.disabled}
                  data-selected={(!multiple && option.selected) || undefined}
                  class="rounded-sm px-1 text-left data-[selected]:bg-selected-background disabled:cursor-not-allowed"
                  onclick={(event) => {
                    event.stopPropagation();
                    option.select();
                  }}>{item.node.name}</button
                >{/snippet}
            </TreeSelectOption>
          {/snippet}
        </DemoTree>
