...
触发器使用 Snippet，接收 props 与 action，绑定 use:action 并展开 props。内容 Snippet 接收 open、close。
...
`createPopover(options)` 是供自定义 Popover 结构使用的公开 Svelte 适配器。它持有稳定的 Core 控制器，并返回 controller, readable snapshot store, and element refs。通过控制器打开/关闭、连接触发/内容/箭头元素并订阅状态；内部 context 读取函数不属于这套独立 API。
