<script lang="ts">
  ...
  const emptySnapshot: SortableControllerSnapshot = {
    activeId: null,
  ...
  function columnGrid(constrained: boolean) {
    ...
    const leaf = (id: string): ColumnDef<Features, Person> =>
      ({
        accessorKey: id,
        size: id === "name" ? 180 : id === "department" ? 170 : 140,
        header: labels[id],
      }) as unknown as ColumnDef<Features, Person>;
      ...
      ? [
          {
            ...
            columns: leaves.filter(
              (column) =>
                ...
                ["name", "department", "status"].includes(
          ...
          ...leaves.filter(
            (column) =>
              "accessorKey" in column &&
              ["id", "age"].includes(String(column.accessorKey)),
          ),
          {
            id: "metrics",
            header: "Metrics",
    ...
    const table = createDataTable({
      ...
      initialState: {
        columnOrder: order,
        ...(constrained
          ? { columnPinning: { start: ["name"], end: ["progress"] } }
          : {}),
      },
    });
    const onChange = (next: string[]) => {
      ...
      if (
        ...
        message = "Rejected: pinned columns cannot be moved or targeted.";
        return;
      }
      if (constrained && groups[source] !== groups[target]) {
        table.setColumnOrder(order);
        message =
          "Rejected: columns must share a real group, or both be ungrouped.";
        return;
      }
      order = [...next];
      table.setColumnOrder(order);
      sortable.updateOptions({ items: order, axis: "x", onChange, onSnapshot });
      message = `Moved ${source} near ${target}.`;
    };
    const onSnapshot = (value: SortableControllerSnapshot) => {
      ...
      table.setColumnOrder(
        value.activeId === null
          ? order
          : (sortable.getPreviewItems() as string[]),
      );
    };
    const sortable = createSortableAction({
      items: order,
      axis: "x",
      onChange,
      onSnapshot,
    ...
    return {
      ...
      headerAction: (
        ...
        (constrained && ["name", "progress"].includes(header.column.id))
          ? undefined
          : sortable.item(node, { id: header.column.id }),
      cellAction: (node: HTMLElement, cell: Cell<Features, Person>) => {
        const cleanup = sortable.controller.registerMotionTarget(
          cell.column.id,
          node,
        );
      ...
      activeHeader: () =>
        typeof snapshot.activeId === "string"
          ? table
              .getFlatHeaders()
              .find((header) => header.column.id === snapshot.activeId)
          : undefined,
      overlayStyle: () => {
  ...
  const free = columnGrid(false),
    locked = columnGrid(true);
  const rowColumns: ColumnDef<Features, Person>[] = [
    { id: "__drag__", header: "", size: 44 },
    { accessorKey: "name", header: "Name" },
    { accessorKey: "department", header: "Department" },
    { accessorKey: "status", header: "Status" },
  ];
  let rowOrder = $state(people5.map((person) => person.id)),
    rowSnapshot = $state(emptySnapshot);
  const rowData = (order: readonly string[] = rowOrder) =>
    order
      .map((id) => people5.find((person) => person.id === id))
      .filter((row): row is Person => row !== undefined);
  let committedRows = rowData();
  const rowTable = createDataTable({
    features,
    data: committedRows,
    columns: rowColumns,
    getRowId: (row) => row.id,
  ...
  const onRowChange = (next: string[]) => {
    rowOrder = [...next];
    rowSortable.updateOptions({
      items: rowOrder,
      axis: "y",
      onChange: onRowChange,
      onSnapshot: updateRowSnapshot,
    });
    syncRowTable();
  };
  const updateRowSnapshot = (value: SortableControllerSnapshot) => {
    ...
    syncRowTable(
      value.activeId === null
        ? rowOrder
        : (rowSortable.getPreviewItems() as string[]),
    );
  };
  const rowSortable = createSortableAction({
    items: untrack(() => rowOrder),
    axis: "y",
    onChange: onRowChange,
    onSnapshot: updateRowSnapshot,
  });
  ...
  const rowAction = (
    node: HTMLElement,
    row: ReturnType<typeof rowTable.getRowModel>["rows"][number],
  ) => rowSortable.item(node, { id: row.id });
  const activeRow = () =>
    typeof rowSnapshot.activeId === "string"
      ? rowTable
          .getRowModel()
          .rows.find((row) => row.id === rowSnapshot.activeId)
      : undefined;
  ...
  const styleToString = (style: object) =>
    (Object.entries(style) as Array<[string, string | number | undefined]>)
...
{#snippet dndHeader(item: Header<Features, Person>)}
  {#if item.column.columns.length}{String(
      item.column.columnDef.header ?? "",
    )}{:else}<span class="inline-flex min-w-0 items-center gap-2"
      ><EllipsisIcon class="size-4 rotate-90 text-muted-foreground" /><span
        class="truncate">{labels[item.column.id]}</span
      ></span
    >{/if}
{/snippet}
{#snippet rowOverlayCell(item: Cell<Features, Person>)}
  {#if item.column.id === "__drag__"}<EllipsisIcon
      class="size-4 rotate-90 text-muted-foreground"
    />{:else}{String(item.getValue() ?? "")}{/if}
{/snippet}
<DemoSection
...
>
  <div class="space-y-3">
    {#each [free, locked] as item (String(item.constrained))}<section
        class="space-y-1.5"
      >
        <h3 class="text-sm font-medium text-foreground">
          {item.constrained
            ? "Column: group boundaries + pinned locked"
            : "Column reorder"}
        </h3>
        <div use:item.container>
          <DataTable
            table={item.table}
            header={dndHeader}
            headerAction={item.headerAction}
            cellAction={item.cellAction}
            getCellProps={(cell) => ({
              style: `${item.motionStyle(cell.column.id)};visibility:${item.snapshot().activeId === cell.column.id ? "hidden" : "visible"}`,
            })}
          />
        </div>
        <p class="text-xs text-muted-foreground">{item.message()}</p>
        {#if item.activeHeader()}<DataTableColumnOverlay
            table={item.table}
            header={item.activeHeader()!}
            headerContent={dndHeader}
            style={`${item.overlayStyle()};height:auto`}
          />{/if}
      </section>{/each}
    <section class="space-y-1.5">
      <h3 class="text-sm font-medium text-foreground">Row reorder</h3>
      <div use:rowContainer>
        <DataTable
          table={rowTable}
          cell={rowOverlayCell}
          {rowAction}
          getRowProps={(row) => ({
            class: rowSnapshot.activeId === row.id ? "opacity-0" : undefined,
            style: styleToString(rowSortable.getItemStyle(row.id)),
          })}
        />
      </div>
      <p class="text-xs text-muted-foreground">
        Current row order: {rowOrder.join(" → ")}
      </p>
      {#if activeRow()}<DataTableRowOverlay
          table={rowTable}
          row={activeRow()!}
          cellContent={rowOverlayCell}
