<script lang="ts">
  ...
  const treeColumns: ColumnDef<Features, Person>[] = [
    { id: "__expand__", header: "", size: 44 },
    { accessorKey: "name", header: "Name" },
    { accessorKey: "department", header: "Department" },
  ];
  const detailColumns: ColumnDef<Features, Person>[] = [
    { id: "__expand__", header: "", size: 44 },
    { accessorKey: "name", header: "Name" },
    { accessorKey: "status", header: "Status" },
  ];
  const tree = createDataTable({
    features,
    data: peopleTree,
    columns: treeColumns,
    getRowId: (row) => row.id,
...
{#snippet cell(item: Cell<Features, Person>, revision: number)}<span
    ...
    >{#if item.column.id === "__expand__"}<Button
        size="icon-sm"
        variant="outline"
        disabled={!item.row.getCanExpand()}
        aria-expanded={item.row.getIsExpanded()}
        onclick={() => item.row.toggleExpanded()}
        >{#if item.row.getIsExpanded()}<MinusIcon
            class="size-4"
          />{:else}<PlusIcon class="size-4" />{/if}</Button
      >{:else if item.column.id === "name"}<span
        style:padding-inline-start={`${item.row.depth * 16}px`}
        >{String(item.getValue() ?? "")}</span
      >{:else}{String(item.getValue() ?? "")}{/if}</span
  >{/snippet}
{#snippet subComponent(row: Row<Features, Person>)}<div
    class="bg-muted-background p-2 text-sm"
  >
    {row.original.name}: {row.original.visits} visits, {row.original.progress}%
    progress.
  </div>{/snippet}
<DemoSection
  title="Row expansion"
  ...
  ><div class="grid gap-3 xl:grid-cols-2">
    <section class="space-y-1.5">
      ...
      <DataTable table={tree} {cell} />
    </section>
    <section class="space-y-1.5">
      <h3 class="text-sm font-medium text-foreground">Detail panel</h3>
      <DataTable table={detail} {cell} {subComponent} />
    </section>
  </div></DemoSection
>
