...
export const avatarContext: InjectionKey<{ controller: ReturnType<typeof createImageLoadingController>; status: Ref<string> }> = Symbol('avatar')
export function createAvatarContext() {
  ...
  controller.subscribe(() => { status.value = controller.getStatus() })
