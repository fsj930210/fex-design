...
| 内容 | JSX.Element children | — | 被附着内容；没有内容时作为独立徽标。 |

## BadgeGroup API

BadgeGroup 沿用 Primitive 语义：children plus `overflow(count, items)`，以及可选的 `maxCount`。

## BadgeRibbon API

BadgeRibbon 使用框架原生输入和 JSX.Element children 区分 Ribbon 标记与被包裹内容；`color` 和 `placement` 沿用 Primitive 语义。
