...
| 内容 | content projection | — | 被附着内容；没有内容时作为独立徽标。 |

## BadgeGroup API

BadgeGroup 沿用 Primitive 语义：projected Badge children plus optional overflow TemplateRef，以及可选的 `maxCount`。

## BadgeRibbon API

BadgeRibbon 使用框架原生输入和 content projection 区分 Ribbon 标记与被包裹内容；`color` 和 `placement` 沿用 Primitive 语义。

## 结构化样式

`classNames` and `styles` inputs 分别作用于 `root`、`content` 和 `indicator`，无需替换组件结构。
