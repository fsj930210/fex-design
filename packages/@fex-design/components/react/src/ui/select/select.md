...
UI Select is the items-driven shortcut. It maps arbitrary data through `fieldNames.value` and `fieldNames.label`; `disabled` remains a boolean field on each item.
