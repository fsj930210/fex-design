...
## Import
    ...
    import { Input, InputPassword, InputSearch, InputGroup } from '@fex-design/svelte/ui/input'
...
| prefix / suffix | Svelte 5 `Snippet` content props. | — | Content inside InputRoot; non-interactive on regular Input. |
| addonBefore / addonAfter | Svelte 5 `Snippet` content props. | — | Content outside the Input focus ring. |
...
| native attributes | Svelte `HTMLInputAttributes` | — | Native input attributes and events. |
...
`bind:ref` exposes the native `HTMLInputElement`.

## Styling

`class`, `classNames`, and native style strings. Use `classNames` / `styles` for semantic parts, selectors for hover/focus/invalid states, and base variables such as `--input-background`, `--input-border-color`, `--input-color`, `--input-placeholder-color`, `--input-ring-color`, and `--input-group-radius` for reusable theming.
