<script lang="ts">
  ...
  import { Popover, PopoverTrigger, PopoverPortal, PopoverContent, PopoverArrow, PopoverHeader, PopoverTitle } from '@fex-design/svelte/primitive/popover';
  import { Button } from '@fex-design/svelte/ui/button';
  type DemoCase = { label: string; options: PopoverOptions }
let sideOffset = $state(12)
let alignOffset = $state(0)
const cases = $derived<DemoCase[]>( (['start', 'center', 'end'] as const).map((align) => ({ label: align, options: { side: 'bottom', align, arrow: true, avoidCollisions: false, sideOffset: sideOffset, alignOffset: alignOffset } })))
...
<div class="grid gap-4">
  ...
  <label>浮层与触发元素距离 {sideOffset}px <input type="range" min="0" max="40" bind:value={sideOffset} /></label>
  <label>面板与触发元素对齐偏移 {alignOffset}px <input type="range" min="-40" max="40" bind:value={alignOffset} /></label>
