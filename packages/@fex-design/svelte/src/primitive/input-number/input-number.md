# Svelte Primitive InputNumber

Composable numeric input built from the existing Input and Button primitives.

## Import

    import { InputNumberRoot, InputNumberControl, InputNumberClear, InputNumberActions, InputNumberIncrement, InputNumberDecrement, useInputNumber } from '@fex-design/svelte/primitive/input-number'
...
Control inherits `HTMLInputAttributes`; native focus, blur, keyboard, aria, data attributes, and element access pass through.

## useInputNumber API

The public logic layer accepts the same options and drives custom DOM without rendering the default structure. Reactive values use rune-backed accessors.
