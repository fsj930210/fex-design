...
`BadgeGroup` 接受 `maxCount`，集合内容使用 children plus `overflow(count, items)`。不传 `maxCount` 时显示全部项目，默认溢出节点为 `+N`。

## BadgeRibbon API

`BadgeRibbon` 接受 `color`、逻辑位置 `placement="start" | "end"` 和 JSX.Element children。默认位置为 `end`，默认颜色为 `primary`。
