<script lang="ts">
  ...
  const { sortable, snapshot, styleToString } =
    getContext<SortableContext>(sortableContextKey);
  const overlayStyle = $derived(
    styleToString((snapshot().motionVersion, sortable.getOverlayStyle())),
  );
