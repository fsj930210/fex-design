...
export function Radio(props: RadioProps) {
  ...
  const [local, control] = splitProps(props, ['children', 'class', 'style', 'classNames', 'styles', 'id'])
  ...
  return (
    ...
    >
      <PrimitiveRadio {...control} id={controlId()} class={local.classNames?.control} style={local.styles?.control} />
      {local.children !== undefined && (
        <label for={controlId()} class={cn(radioLabelClassName, local.classNames?.label)} style={local.styles?.label}>
          {local.children}
        </label>
      )}
    </div>
...
export interface RadioOption { label: JSX.Element; value: RadioValue; disabled?: boolean }
export interface RadioGroupProps extends PrimitiveGroupProps { options?: readonly RadioOption[] }
export function RadioGroup(props: RadioGroupProps) {
  const [local, rest] = splitProps(props, ['options', 'children'])
  return (
    <PrimitiveRadioGroup {...rest}>
      {local.options ? (
        <For each={local.options}>{(option) => <Radio value={option.value} disabled={option.disabled}>{option.label}</Radio>}</For>
      ) : local.children}
    </PrimitiveRadioGroup>
  )
}
export interface RadioButtonGroupProps extends PrimitiveGroupProps { options?: readonly RadioOption[] }
export function RadioButtonGroup(props: RadioButtonGroupProps) {
  const [local, rest] = splitProps(props, ['options', 'children', 'class'])
  return (
    <PrimitiveRadioGroup {...rest} orientation={rest.orientation ?? 'horizontal'} class={cn('gap-0', local.class)}>
      {local.options ? (
        <For each={local.options}>{(option) => <RadioButton value={option.value} disabled={option.disabled}>{option.label}</RadioButton>}</For>
      ) : local.children}
    </PrimitiveRadioGroup>
  )
}
