<script lang="ts">
  ...
  import { BadgeGroup } from "@fex-design/svelte/primitive/badge";

  const colors = ["primary", "info", "success", "warning", "danger"] as const;
  const overflowItems = ["Design", "Frontend", "Backend", "QA", "Operations"];
</script>

{#snippet overflowBadge(item: string)}<Badge color="info">{item}</Badge
  >{/snippet}
  ...
  <div class="mx-auto w-full max-w-5xl space-y-4">
    <header class="space-y-2">
      ...
      <p class="max-w-2xl text-sm leading-6 text-muted-foreground">
        用颜色表达状态，并用 maxCount 收起过多的标签。
      </p>
    </header>
    <div class="grid gap-4">
      <Card title="Colors" description="Badge 的内置语义颜色。"
        ><div class="flex min-w-0 flex-wrap items-center gap-2">
          {#each colors as color}<Badge {color}>{color}</Badge>{/each}
        </div></Card
