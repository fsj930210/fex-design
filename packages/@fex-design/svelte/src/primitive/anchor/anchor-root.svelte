<script lang="ts">
  ...
  interface Props extends Omit<HTMLAttributes<HTMLElement>, "onchange"> {
    ...
    container?:
      Window | HTMLElement | (() => Window | HTMLElement | null | undefined);
    targetOffset?: number;
    threshold?: number;
  ...
  let highlightedKeys = $derived.by(() => {
    const result = new Set(currentKeys);
  ...
  function refreshIndicator() {
    if (!root) return;
    ensureAnchorLinkVisible(root, currentKeys, orientation);
    inkStyles = getAnchorIndicatorStyles(root, currentKeys, orientation);
  }
  function refresh() {
  ...
  function registerItem(item: AnchorRegisteredItem) {
...
<nav
  ...
  class={cn(anchorRootClassName({ orientation }), className)}
>
  {@render children?.()}
</nav>
