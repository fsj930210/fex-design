...
Content follows Svelte's native model: `children?: Snippet`. Place `TagAction` inside `Tag` for an inline operation. It defaults to XIcon, while custom content can represent remove, edit, more, or another action. `TagAction` does not prescribe behavior or coordinate the parent disabled state.
