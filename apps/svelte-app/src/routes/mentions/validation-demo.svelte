<script lang="ts">
  ...
  const validators = {
    onSubmit: ({ value }: { value: string }) =>
      value.trim() ? undefined : "Prompt is required.",
  };
...
<Card
  ...
  description="Invalid state drives the default Textarea styling."
>
    ...
    <Field name="prompt" {validators}>
      {#snippet children(field)}
        ...
        <FieldRoot required invalid={hasError} {hasError}>
          <FieldLabel
            >{#snippet children()}Prompt <FieldRequiredIndicator
              />{/snippet}</FieldLabel
          >
          <FieldControl>
            {#snippet children()}
              ...
              >
                <MentionsTrigger
                  placeholder="Submit empty content to see validation"
                />
                <MentionsContent>
                  <MentionsList>
          ...
          {#if hasError}<FieldError
              >{#snippet children()}{errors(field).join(
                  ", ",
                )}{/snippet}</FieldError
            >{/if}
        </FieldRoot>
