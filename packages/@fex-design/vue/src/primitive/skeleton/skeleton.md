# Vue Primitive Skeleton

Styled, independently composable Skeleton placeholders. Every component uses a native `div` host.

## Import

    import { SkeletonAvatar, SkeletonBlock, SkeletonButton, SkeletonImage, SkeletonInput, SkeletonText } from '@fex-design/vue/primitive/skeleton'
...
| native attributes | Vue div attrs | — | Native div attributes and events pass through, including `class`, `style`, `data-*`, and `aria-*`. |
| element access | framework-native API | — | Use a template ref to access the native `HTMLDivElement`. |
