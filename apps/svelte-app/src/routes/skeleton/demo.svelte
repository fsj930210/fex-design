<script lang="ts">
  ...
  let {
    ...
    kind: "basic" | "avatar" | "card" | "text" | "form" | "table" | "animation";
  } = $props();
</script>
  ...
  <Card title="Table" description="Repeat stable row shapes for data loading."
    ><div class="grid max-w-2xl gap-2">
      <div class="grid grid-cols-3 gap-3 border-b pb-2">
        <Skeleton class="h-4 w-24" /><Skeleton class="h-4 w-20" /><Skeleton
          class="h-4 w-16"
        />
      </div>
      {#each ["w-2/5", "w-3/5", "w-1/2"] as width (width)}<div
          class="grid grid-cols-3 gap-3"
        >
          <div class="flex items-center gap-1.5">
            <Skeleton class="size-8 rounded-full" /><Skeleton
              class={["h-4", width]}
            />
          </div>
