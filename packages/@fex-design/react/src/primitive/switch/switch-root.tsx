...
export function SwitchRoot({
  checked, defaultChecked = false, disabled = false, loading = false,
  size = 'md', shape = 'rounded', className, onClick, onChange, ...props
}: SwitchRootProps) {
  ...
  return (
    <button {...props} type="button" role="switch" disabled={disabled || loading}
      aria-checked={currentChecked} aria-busy={loading || undefined}
      data-slot="switch" data-state={currentChecked ? 'checked' : 'unchecked'}
      data-loading={loading ? '' : undefined} data-disabled={disabled ? '' : undefined}
