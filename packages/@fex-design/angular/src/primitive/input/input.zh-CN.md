...
原生契约：`input[inputControl]` 上的原生属性和事件。`InputControl.element` 暴露原生 input；UI Input 暴露 `focus()`、`blur()` 和 `select()`。
...
使用原生 `class`、`classNames` 和 style 绑定。 hover、focus-within、data 属性和 `[aria-invalid=true]` 等状态使用选择器覆盖，CSS Variables 只保留可复用基础 token。
