<script lang="ts" generics="TItem extends TransferDataItem">
  ...
  import ChevronLeftIcon from '@fex-design/svelte/icons/chevron-left.svelte';
  import ChevronRightIcon from '@fex-design/svelte/icons/chevron-right.svelte';
  import ChevronsLeftIcon from '@fex-design/svelte/icons/chevrons-left.svelte';
  import ChevronsRightIcon from '@fex-design/svelte/icons/chevrons-right.svelte';
  ...
  > extends TransferControllerOptions<TItem> {
    ...
    sourceHeader?: Snippet<[TransferPanelApi<TItem>]>;
    sourceBody?: Snippet<[TransferPanelApi<TItem>]>;
    sourceFooter?: Snippet<[TransferPanelApi<TItem>]>;
    targetHeader?: Snippet<[TransferPanelApi<TItem>]>;
    targetBody?: Snippet<[TransferPanelApi<TItem>]>;
    targetFooter?: Snippet<[TransferPanelApi<TItem>]>;
    actions?: Snippet<
      [
        ReturnType<typeof createTransferController<TItem>>,
        TransferSnapshot<TItem>,
      ]
    >;
    item?: Snippet<[TItem, TransferSide]>;
    validation?:
      { status: "error" | "warning"; message: string | undefined } | undefined;
    class?: string;
  }
  let {
    items,
  ...
  $effect(() => {
    controller.updateOptions(currentOptions());
  });
  const fields = $derived(resolveTransferFieldNames(fieldNames));
  function api(side: TransferSide): TransferPanelApi<TItem> {
    const source = side === "source";
    const keys = source
      ? $snapshot.sourceCheckedKeys
      : $snapshot.targetCheckedKeys;
    return {
      side,
      items: source ? $snapshot.sourceItems : $snapshot.targetItems,
      checkedKeys: keys,
      controller,
      setCheckedKeys: source
        ? controller.setSourceCheckedKeys
        : controller.setTargetCheckedKeys,
      isChecked: (key) => keys.includes(key),
    };
  }
  function enabledKeys(side: TransferSide) {
    return api(side)
      .items.filter((entry) => !readTransferDisabled(entry, fields))
      .map((entry) => readTransferKey(entry, fields));
  }
  function checkedState(side: TransferSide) {
    const keys = enabledKeys(side);
    const count = keys.filter((key) =>
      api(side).checkedKeys.includes(key),
    ).length;
  ...
  function change(side: TransferSide, value: unknown) {
    api(side).setCheckedKeys(
      Array.isArray(value)
        ? (value as TransferKey[])
        : value == null
          ? []
          : [value as TransferKey],
    );
...
>
  <div data-slot="transfer-layout" class={transferLayoutClassName}>
    {#each ["source", "target"] as side (side)}
      {@const typedSide = side as TransferSide}{@const panel =
        api(typedSide)}{@const header =
        typedSide === "source" ? sourceHeader : targetHeader}{@const body =
        typedSide === "source" ? sourceBody : targetBody}{@const footer =
        typedSide === "source" ? sourceFooter : targetFooter}
      <section
        ...
        class={cn(
          typedSide === "source"
            ...
            : transferTargetPanelClassName,
          "row-start-1",
          typedSide === "source" ? "col-start-1" : "col-start-3",
        )}
      >
        <header
        ...
        >
          {#if header}{@render header(panel)}{:else}<Checkbox
              checked={checkedState(typedSide)}
              disabled={disabled || enabledKeys(typedSide).length === 0}
              class={checkboxClassName()}
              aria-label={`Select all ${title[typedSide] ?? (typedSide === "source" ? "Source" : "Target")}`}
              onCheckedChange={(next) => toggleAll(typedSide, next === true)}
              >{#snippet children(current)}<CheckboxIndicator
                  checked={current}
                  class={checkboxIndicatorClassName}
                  >{#snippet children()}<CheckIcon
                      class={checkboxCheckIconClassName}
                    /><MinusIcon
                      class={checkboxMinusIconClassName}
                    />{/snippet}</CheckboxIndicator
                >{/snippet}</Checkbox
            ><span class="min-w-0 flex-1 truncate font-medium"
              >{title[typedSide] ??
                (typedSide === "source" ? "Source" : "Target")}</span
            ><span class="shrink-0 text-muted-foreground"
              >{panel.checkedKeys.length}/{panel.items.length}</span
            >{/if}
        </header>
        <div data-slot="transfer-panel-body" class={transferPanelBodyClassName}>
          {#if body}{@render body(panel)}{:else}<Listbox
              data-variant="transfer"
              multiple
              {disabled}
              items={panel.items}
              value={panel.checkedKeys}
              getItemValue={(entry) => readTransferKey(entry, fields)}
              getItemDisabled={(entry) =>
                disabled || readTransferDisabled(entry, fields)}
              onChange={(value) => change(typedSide, value)}
              class={listboxRootClassName({ variant: "transfer" })}
              >{#snippet children()}{#each panel.items as entry (readTransferKey(entry, fields))}{@const key =
                    readTransferKey(entry, fields)}<ListboxItem
                    value={key}
                    ...
                    >{#snippet children()}<span
                        ...
                        >{#if panel.isChecked(key)}<CheckIcon
                            class="size-3"
                          />{/if}</span
                      ><span class="min-w-0 flex-1 truncate text-sm"
                        >{#if item}{@render item(
                            entry,
                            typedSide,
                          )}{:else}{entry[fields.label]}{/if}</span
                      >{/snippet}</ListboxItem
                  >{/each}{/snippet}</Listbox
            >{/if}
        </div>
        {#if footer}<footer
    ...
    >
      {#if actions}{@render actions(controller, $snapshot)}{:else}<Button
          class={buttonClassName({ variant: "outlined", size: "icon-md" })}
          disabled={!can("target")}
          aria-label="Move selected to target"
          onclick={controller.moveToTarget}
          >{#snippet children()}<ChevronRightIcon />{/snippet}</Button
        ><Button
          class={buttonClassName({ variant: "outlined", size: "icon-md" })}
          disabled={!can("source")}
          aria-label="Move selected to source"
          onclick={controller.moveToSource}
          >{#snippet children()}<ChevronLeftIcon />{/snippet}</Button
        ><Button
          class={buttonClassName({ variant: "outlined", size: "icon-md" })}
          disabled={!can("allTarget")}
          aria-label="Move all to target"
          onclick={controller.moveAllToTarget}
          >{#snippet children()}<ChevronsRightIcon />{/snippet}</Button
        ><Button
          class={buttonClassName({ variant: "outlined", size: "icon-md" })}
          disabled={!can("allSource")}
          aria-label="Move all to source"
          onclick={controller.moveAllToSource}
          >{#snippet children()}<ChevronsLeftIcon />{/snippet}</Button
        >{/if}
    </div>
  </div>
  {#if validation}<div
      ...
      class={validation.status === "warning"
        ? transferWarningMessageClassName
        : transferMessageClassName}
    >
      {validation.message}
