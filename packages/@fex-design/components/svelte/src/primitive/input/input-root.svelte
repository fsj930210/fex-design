<script lang="ts">
  ...
  > {
    ...
    action?:
      ((element: HTMLElement) => { destroy?: () => void } | void) | undefined;
    onValueChange?:
      | ((
          value: string,
          meta: { reason: InputChangeReason; event?: Event },
        ) => void)
      | undefined;
...
<div
  ...
  class={cn(inputRootClassName({ size, variant }), className)}
>
  {@render children?.()}
</div>
