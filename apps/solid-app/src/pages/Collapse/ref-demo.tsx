...
export function RefCollapseDemo() {
  ...
  return (
    <Card
      ...
      description="The ref callback exposes methods for this Collapse instance."
    >
      <div class="mb-2 flex flex-wrap gap-1.5">
        <Button variant="outline" size="sm" onClick={() => collapseRef?.expand('security')}>
          Open security
        </Button>
        <Button variant="outline" size="sm" onClick={() => collapseRef?.toggle('billing')}>
          Toggle billing
        </Button>
        <Button variant="ghost" size="sm" onClick={() => collapseRef?.clear()}>
          Clear
        </Button>
      </div>
