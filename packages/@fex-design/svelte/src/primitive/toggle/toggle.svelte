<script lang="ts">
  ...
  interface Props
    extends
      Omit<HTMLButtonAttributes, "children" | "onchange" | "value">,
      ToggleStyleProps {
    pressed?: boolean;
  ...
  const group = getContext<ToggleGroupContextValue | undefined>(
    toggleGroupContextKey,
  );
  let internalPressed = $state(defaultPressed);
  const inGroup = $derived(group !== undefined && value !== undefined);
  const currentPressed = $derived(
    inGroup ? group!.isPressed(value!) : (pressed ?? internalPressed),
  );
  const currentDisabled = $derived(
    disabled === true || (group?.disabled() ?? false),
  );
</script>
