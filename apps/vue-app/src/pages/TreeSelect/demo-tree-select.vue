...
import {
  ...
  TreeSelectRoot,
  TreeSelectTrigger,
} from '@fex-design/vue/primitive/tree-select'
...
<template>
  ...
  >
    <TreeSelectTrigger v-slot="{ triggerProps, triggerRef, inputProps, selectedItems, clear }">
      <div
        :ref="triggerRef"
        v-bind="inputRootTriggerProps(triggerProps)"
        class="w-80"
      ...
      >
        ...
        >
          <template v-if="props.multiple"
            ><template v-if="selectedItems.length <= (props.maxTagCount ?? 2)"
              ><Tag
                ...
                @close.stop="
                  root.controller.setValues(
                    root.state.values.filter((value: TreeSelectValue) => value !== item.value),
                  )
                "
                >{{ item.label }}</Tag
              ></template
            ><Tag v-else size="sm" class="ml-1.5"
              >已选择 {{ selectedItems.length }} 项</Tag
            ></template
          >
          <InputControl
            :read-only="Boolean(inputProps.readonly)"
            :placeholder="
              selectedItems.length ? undefined : props.searchable ? '搜索部门' : '请选择部门'
            "
            @input="inputProps.onInput"
          />
          <InputClear
            :force-mount="selectedItems.length > 0 || Boolean(props.searchValue)"
            aria-label="清除"
            @click.stop.prevent="
              clear()
              emit('clear')
      ...
      <div :class="hasCustomContent ? 'hidden' : undefined">
        <DemoTree
          ...
          :on-checked-keys-change="
            props.multiple
              ? (keys: readonly TreeKey[], meta: { changedKeys: readonly TreeKey[] }) => {
                  if (!props.checkStrictly) {
                    root.controller.setValues(keys)
                    return
                  }
                  const next = new Set(root.state.values)
                  meta.changedKeys.forEach((key) =>
                    keys.includes(key) ? next.add(key) : next.delete(key),
                  )
                  root.controller.setValues([...next])
                }
              : undefined
          "
        ...
        >
          <template #title="{ item }">
            ...
            >
              ...
              >
                <span
                  v-for="(part, index) in highlightParts(String(item.node.name))"
                  :key="index"
                  :class="part.match ? 'bg-warning/30' : undefined"
                  >{{ part.text }}</span
                >
              </button>
            </TreeSelectOption>
