...
| 原生属性 | Svelte element attributes and bindable element access | — | 原生属性、事件与元素访问。 |
...
| 原生属性 | Svelte element attributes and bindable element access | — | 原生 li 属性与事件。 |

## 原生部件

AnchorLink 使用原生 button；AnchorList、AnchorRail 与 AnchorIndicator 保留各自宿主的原生属性。事件：`onchange`；AnchorLink 的原生 `onclick`。

## 自定义 UI 逻辑

Primitive 组件提供组合契约；内部 context 模块不对外导出。
