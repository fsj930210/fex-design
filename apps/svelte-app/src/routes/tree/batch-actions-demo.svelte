<script lang="ts">
  ...
  const controller = createTreeController<DepartmentNode>({
    ...
    features: [
      expansionFeature(),
      selectionFeature({ multiple: true }),
      checkFeature(),
    ],
  });
</script>
...
<Card
  ...
  ><div class="mb-2 flex flex-wrap gap-1.5">
    <Button
      ...
      variant="outline"
      onclick={() =>
        controller.getFeature<ExpansionFeatureApi>("expansion")?.expandAll()}
      >Expand all</Button
    ><Button
      size="sm"
      variant="outline"
      onclick={() =>
        controller.getFeature<ExpansionFeatureApi>("expansion")?.collapseAll()}
      >Collapse all</Button
    ><Button
      size="sm"
      variant="outline"
      onclick={() =>
        controller.getFeature<SelectionFeatureApi>("selection")?.selectAll()}
      >Select all</Button
    ><Button
      size="sm"
      variant="outline"
      onclick={() =>
        controller.getFeature<SelectionFeatureApi>("selection")?.clear()}
      >Clear selection</Button
    ><Button
      size="sm"
      variant="outline"
      onclick={() =>
        controller.getFeature<CheckFeatureApi>("check")?.checkAll()}
      >Check all</Button
    ><Button
      size="sm"
      variant="outline"
