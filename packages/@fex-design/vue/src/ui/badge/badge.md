...
## Import
    ...
    import { Badge, BadgeGroup, BadgeRibbon } from '@fex-design/vue/ui/badge'
...
| content | default slot | — | Attached content; without content the badge is standalone. |

## BadgeGroup API

BadgeGroup keeps Primitive semantics: default slot plus scoped `overflow` slot with `{ count, items }` and optional `maxCount`.

## BadgeRibbon API

BadgeRibbon separates the ribbon marker from wrapped content using framework-native inputs and default slot; `color` and `placement` keep Primitive Ribbon semantics.

## Structured styling

`classNames` and `styles` objects target `root`, `content`, and `indicator` without replacing the component structure.
