...
原生契约：`JSX.InputHTMLAttributes<HTMLInputElement>`。`ref` 返回原生 `HTMLInputElement`。
...
使用 `class`、`classNames` 和 `JSX.CSSProperties` 样式。 hover、focus-within、data 属性和 `[aria-invalid=true]` 等状态使用选择器覆盖，CSS Variables 只保留可复用基础 token。
