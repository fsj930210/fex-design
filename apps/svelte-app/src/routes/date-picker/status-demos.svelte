...
>
  <FieldRoot invalid hasError hasDescription>
    ...
    <DatePickerRoot status="error">
      <DatePickerTrigger status="error" class="w-56" placeholder="请选择日期" />
      <DatePickerContent class="overflow-hidden p-0">
        <DatePickerPanel />
      </DatePickerContent>
    </DatePickerRoot>
    <FieldDescription
      >状态来自 Form / Field，不由 DatePicker 自己校验。</FieldDescription
    >
  ...
  <FieldRoot hasDescription>
    ...
    <RangePickerRoot status="warning">
      <RangePickerTrigger
        status="warning"
        class="w-80"
    ...
    <FieldDescription
      >warning 只改变视觉状态，提交仍交给外层 Form。</FieldDescription
    >
  </FieldRoot>
</Card>
