<script lang="ts">
  ...
  let cascadeKeys: (string | number)[] = $state([]);
  let strictKeys: (string | number)[] = $state([]);
</script>
  ...
  ><div class="grid gap-3 lg:grid-cols-2">
    <div class="space-y-1.5">
      ...
      <DemoTree
        ...
        onCheckedKeysChange={(keys) => (cascadeKeys = [...keys])}
    ...
    <div class="space-y-1.5">
      ...
      <DemoTree
        ...
        onCheckedKeysChange={(keys) => (strictKeys = [...keys])}
        class="rounded-md border border-border bg-background p-1.5"
      />
    </div>
  </div></Card
