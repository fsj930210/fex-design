...
<div class="grid gap-4 md:grid-cols-2">
  ...
  >
    <AutoCompleteRoot items={users} {fieldNames}>
      <AutoCompleteTrigger
        placeholder="Custom suggestion rows"
        clearable
        status="warning"
      />
      <AutoCompleteContent>
          ...
          {#snippet item(user, state)}
            <div>
              ...
              <div class="text-xs text-muted-foreground">
                {user.department} · {user.email}{state.disabled
                  ? " · unavailable"
                  : ""}
              </div>
            </div>
  ...
  >
    <AutoCompleteRoot items={users} {fieldNames}>
      <AutoCompleteTrigger
        name="reviewer"
        required
        aria-describedby="reviewer-error-svelte"
    ...
    <p id="reviewer-error-svelte" class="mt-1.5 text-xs text-danger">
