<script lang="ts">
  ...
  import {
    ...
    type DisabledTime,
    type TimeValue,
  } from "@fex-design/svelte/primitive/time-picker";
...
>
  <TimePickerTrigger class="w-56" {invalid}>
    ...
    {#snippet suffix()}{#if calendarSuffix}<CalendarIcon
          class="size-4"
        />{:else}<ClockIcon class="size-4" />{/if}{/snippet}
  </TimePickerTrigger>
  <TimePickerContent>
    <TimePickerPanel
      >{#each columns as column (column.unit)}{#if column.unit === "hour"}<TimePickerHourColumn
            step={step?.hour}
          />{:else if column.unit === "minute"}<TimePickerMinuteColumn
            step={step?.minute}
          />{:else if column.unit === "second"}<TimePickerSecondColumn
            step={step?.second}
          />{:else}<TimePickerPeriodColumn />{/if}{/each}</TimePickerPanel
    >
    {#if panelExtra}<div class="border-t border-border p-1.5 text-sm">
        <button type="button" class="text-primary">选择当前时间</button>
      </div>{/if}
