...
<template>
  <div class="w-full flex items-center justify-center min-h-[600px] py-16">
    <div class="grid w-full gap-4 overflow-visible">
      ...
      >
        <div v-for="item in cases" :key="item.placement" :style="{ gridArea: item.cell }">
          <TooltipRoot :placement="item.placement" :avoid-collisions="false"
            ><TooltipTrigger v-slot="trigger"
              ><Button v-bind="trigger.props" :ref="trigger.ref" variant="outlined">{{
                item.placement
              }}</Button></TooltipTrigger
            ><TooltipPortal
              ><TooltipContent>{{ item.placement }}<TooltipArrow /></TooltipContent></TooltipPortal
          ></TooltipRoot>
        </div>
      </div>
    </div>
