<script lang="ts">
  ...
  let expandedKeys: (string | number)[] = $state(["company", "engineering"]);
  let selectedKeys: (string | number)[] = $state([]);
</script>
...
<Card
  ...
  ><div class="mb-2 flex flex-wrap gap-1.5">
    <Button
      size="sm"
      variant="outline"
      onclick={() => (expandedKeys = ["company", "engineering", "product"])}
      >Expand departments</Button
    ><Button
      size="sm"
      variant="outline"
      onclick={() => {
        expandedKeys = ["company", "engineering"];
        selectedKeys = ["frontend"];
      }}>Select Frontend</Button
    >
  </div>
  <DemoTree
    ...
    {expandedKeys}
    onExpandedKeysChange={(keys) => (expandedKeys = [...keys])}
    {selectedKeys}
