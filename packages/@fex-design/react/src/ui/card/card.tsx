...
export interface CardProps extends Omit<ComponentProps<'div'>, 'title'>, CardOptions<ReactNode, React.CSSProperties> {}
    ...
    >
      {header ?? (title || description || extra ? (
        <CardHeader className={classNames?.header} style={styles?.header}>
          ...
          {extra ? <CardExtra className={classNames?.extra} style={styles?.extra}>{extra}</CardExtra> : null}
        </CardHeader>
      ) : null)}
