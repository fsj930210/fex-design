...
## Import
    ...
    import { Badge, BadgeGroup, BadgeRibbon } from '@fex-design/solid/ui/badge'
...
| content | JSX.Element children | — | Attached content; without content the badge is standalone. |

## BadgeGroup API

BadgeGroup keeps Primitive semantics: children plus `overflow(count, items)` and optional `maxCount`.

## BadgeRibbon API

BadgeRibbon separates the ribbon marker from wrapped content using framework-native inputs and JSX.Element children; `color` and `placement` keep Primitive Ribbon semantics.
