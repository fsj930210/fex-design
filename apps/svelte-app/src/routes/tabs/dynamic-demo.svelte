<script lang="ts">
  ...
  let items = $state([...initialTabs]);
  let value = $state("overview");
  let nextIndex = $state(4);
  function add() {
  ...
  function remove(target: string) {
    ...
    items = items.filter((item) => item.value !== target);
    if (value === target)
      value = items[Math.min(index, items.length - 1)]?.value ?? "";
  }
</script>
...
<Card
  ...
  ><TabsRoot
    ...
    ><div class="flex min-w-0 items-center gap-1.5">
      <span class="text-xs text-muted-foreground">Workspace</span><TabsList
        class="min-w-0 flex-1"
        >{#each items as item (item.value)}<TabsItem
            value={item.value}
            closable={item.closable}>{item.label}</TabsItem
          >{/each}</TabsList
      ><button
