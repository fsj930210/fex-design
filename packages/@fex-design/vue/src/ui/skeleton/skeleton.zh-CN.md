...
| styles | SkeletonStyles<`StyleValue`> | — | 分别设置 root、avatar、title、paragraph 的行内样式。 |
| 原生属性 | Vue div attrs | — | 骨架状态下透传到根 div；`class`、`style` 与 root 结构化配置合并。 |
...
默认插槽提供真实内容，`#placeholder` 插槽提供自定义骨架。
