...
| prefix / suffix          | 通过内容属性传入 Solid `JSX.Element`。                   | —            | InputRoot 内部内容；普通 Input 中不承担交互。                    |
| addonBefore / addonAfter | 通过内容属性传入 Solid `JSX.Element`。                   | —            | 位于 Input 聚焦环之外的附加内容。                                |
| classNames               | 结构区域 class 映射                                      | —            | root、control、prefix、suffix、clear、addon 和 action 的 class。 |
| styles                   | 结构区域 style 映射                                      | —            | 相同语义区域的框架原生 style。                                   |
| 原生属性                 | `JSX.InputHTMLAttributes<HTMLInputElement>`              | —            | 原生 input 属性与事件。                                          |
...
`ref` 返回原生 `HTMLInputElement`。

## 样式扩展

使用 `class`、`classNames` 和 `JSX.CSSProperties` 样式。 使用 `classNames` / `styles` 定位语义区域，使用选择器覆盖 hover、focus 和错误状态；可复用主题使用 `--input-background`、`--input-border-color`、`--input-color`、`--input-placeholder-color`、`--input-ring-color` 和 `--input-group-radius` 等基础变量。
