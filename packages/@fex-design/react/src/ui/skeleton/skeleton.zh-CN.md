...
| styles | SkeletonStyles<`CSSProperties`> | — | 分别设置 root、avatar、title、paragraph 的行内样式。 |
| 原生属性 | `ComponentProps<'div'>` | — | 骨架状态下透传到根 div；`class`、`style` 与 root 结构化配置合并。 |
...
`children` 提供真实内容，`placeholder` 属性提供自定义骨架。
