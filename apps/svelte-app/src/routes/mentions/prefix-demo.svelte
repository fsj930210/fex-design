...
<Card
  title="Trigger characters"
  description="Each prefix renders a caller-owned list."
>
  <MentionsRoot
    {value}
    prefix={["@", "#", "/"]}
      ...
      <MentionsPrefixCase prefix="#">
        <MentionsList>
          {#each mentionDocs as doc (doc.id)}
            <MentionsItem itemKey={doc.id} value={doc.title}
              >{#snippet children()}#{doc.title}{/snippet}</MentionsItem
            >
          {/each}
        </MentionsList>
      </MentionsPrefixCase>
      <MentionsPrefixCase prefix="/">
        <MentionsList>
          {#each mentionCommands as command (command.id)}
            <MentionsItem itemKey={command.id} value={command.id}
              >{#snippet children()}/{command.label}{/snippet}</MentionsItem
            >
          {/each}
        </MentionsList>
      </MentionsPrefixCase>
