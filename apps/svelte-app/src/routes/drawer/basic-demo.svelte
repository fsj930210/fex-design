...
<Drawer>
  <DrawerTrigger
    >{#snippet children(props)}<Button {...props}>Open drawer</Button
      >{/snippet}</DrawerTrigger
  >
  <DrawerPortal
    ><DrawerMask /><DrawerContent aria-label="Basic drawer"
      ><DrawerHeader>Basic drawer<DrawerClose /></DrawerHeader><DrawerBody
        >Drawer content is composed from independent primitives.</DrawerBody
      ></DrawerContent
    ></DrawerPortal
  >
</Drawer>
