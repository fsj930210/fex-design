...
| 内容 | children Snippet | — | 未传 `count` 时展示的内容。 |
| 原生属性 | HTMLAttributes<HTMLSpanElement> | — | native element attributes。 |
...
`BadgeGroup` 接受 `maxCount`，集合内容使用 `items`, required `item` Snippet, and optional `overflow` Snippet。不传 `maxCount` 时显示全部项目，默认溢出节点为 `+N`。

## BadgeRibbon API

`BadgeRibbon` 接受 `color`、逻辑位置 `placement="start" | "end"` 和 children Snippet。默认位置为 `end`，默认颜色为 `primary`。
