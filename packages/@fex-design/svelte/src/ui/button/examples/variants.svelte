...
<div class="grid gap-3">
  {#each colors as [color, label] (color)}
    <div class="grid grid-cols-[8.5rem_minmax(0,1fr)] items-center gap-4">
      ...
      <div class="flex flex-wrap items-center gap-3">
        {#each variants as variant (variant)}<Button {variant} {color}
            >{variant}</Button
          >{/each}
      </div>
    </div>
  ...
  <div class="grid grid-cols-[8.5rem_minmax(0,1fr)] items-center gap-4">
    ...
    <div class="flex flex-wrap items-center gap-3">
      {#each variants as variant (variant)}<Button {variant} style={customColor}
          >{variant}</Button
        >{/each}
    </div>
  </div>
  <div class="grid grid-cols-[8.5rem_minmax(0,1fr)] items-center gap-4">
    ...
    ><Button
      class="w-fit border-transparent bg-linear-to-r from-violet-600 to-cyan-500 text-white hover:brightness-110"
      >Gradient</Button
    >
  </div>
</div>
