<script lang="ts">
  ...
  const columns: ColumnDef<Features, Person>[] = fields.map((field) => ({
    accessorKey: field,
    header: field,
...
{#snippet header(item: Header<Features, Person>)}
  <span class="inline-flex items-center gap-1">
    ...
    <Button
      size="icon-xs"
      variant="ghost"
      aria-label={`Pin ${item.column.id} to start`}
      onclick={() => item.column.pin("start")}
      ><ChevronLeftIcon class="size-3.5" /></Button
    >
    <Button
      size="icon-xs"
      variant="ghost"
      aria-label={`Unpin ${item.column.id}`}
      onclick={() => item.column.pin(false)}
      ><MinusIcon class="size-3.5" /></Button
    >
    <Button
      size="icon-xs"
      variant="ghost"
      aria-label={`Pin ${item.column.id} to end`}
      onclick={() => item.column.pin("end")}
