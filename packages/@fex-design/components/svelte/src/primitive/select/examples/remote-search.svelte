<script lang="ts">
import SelectRoot from '@fex-design/svelte/primitive/select'; import SelectTrigger from '@fex-design/svelte/primitive/select-trigger'; import SelectContent from '@fex-design/svelte/primitive/select-content'
...
<SelectRoot {options} defaultValue="beijing" showSearch {loading} onSearch={(keyword)=>void search(keyword)}><SelectTrigger class="w-72" placeholder="输入城市或拼音"/><SelectContent loadingText="正在查询城市…" emptyText="未找到城市"/></SelectRoot>
