<script lang="ts">
  ...
  const presets: readonly TagPresetColor[] = [
    "primary",
    "success",
    "warning",
    "danger",
...
<div class="grid gap-4">
  {#each variants as variant (variant)}<section class="grid gap-2">
      <h4 class="text-sm font-medium">预设颜色（{variant}）</h4>
      <div class="flex flex-wrap gap-2">
        {#each presets as color (color)}<Tag {color} {variant}>{color}</Tag
          >{/each}
      </div>
    </section>{/each}{#each variants as variant (variant)}<section
      class="grid gap-2"
    >
      <h4 class="text-sm font-medium">自定义颜色（{variant}）</h4>
      <div class="flex flex-wrap gap-2">
        {#each customColors as color (color)}<Tag {color} {variant}>{color}</Tag
          >{/each}
      </div>
    </section>{/each}
</div>
