<script lang="ts">
  import {
    ...
    MasonryRoot,
    MasonryViewport,
  } from "@fex-design/svelte/primitive/masonry";
...
<div class="grid gap-2">
  ...
  >
    ...
    ><Button size="sm" variant="outline" onclick={switchTarget}>切换目标</Button
    ><Button
      size="sm"
      onclick={() => (detailCount = Math.min(details.length, detailCount + 1))}
      >追加内容</Button
    ><Button
      size="sm"
      variant="outline"
      disabled={!detailCount}
      onclick={() => (detailCount = Math.max(0, detailCount - 1))}
      >收起内容</Button
    ><Button
      size="sm"
      variant="outline"
      onclick={() => (detailCount = details.length)}>连续更新</Button
    ><Button size="sm" variant="ghost" onclick={() => (detailCount = 0)}
      >重置</Button
    >
  </div>
  <MasonryRoot columns={{ minColumnWidth: 220, max: 4 }} gap={16}
    ><MasonryViewport
      >{#each items as item, index (item.id)}{@const active =
          index === target}<MasonryItem itemKey={item.id} {index}
          ><article
          ...
          >
            <div class="flex items-center justify-between gap-1.5">
              <strong>{item.title}</strong><span
                ...
                >{active ? "观察中" : `第 ${index + 1} 项`}</span
              >
            </div>
            <p class="text-sm text-muted-foreground">{summaries[index]}</p>
            {#if active}{#each details.slice(0, detailCount) as detail (detail)}<p
                  class="border-t border-border pt-1.5 text-sm"
                >
                  {detail}
                </p>{/each}{/if}
          </article></MasonryItem
        >{/each}</MasonryViewport
    ></MasonryRoot
  >
</div>
