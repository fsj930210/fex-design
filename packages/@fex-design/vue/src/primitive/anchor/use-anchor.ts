...
export function useAnchor(options: UseAnchorOptions) {
  ...
  const activeKeys = computed(() => {
    void snapshot.value
    return options.activeKeys() ?? controller.getSnapshot().activeKeys
  })
  const orientation = computed(options.orientation)
  const visibleItems = computed(() =>
    orientation.value === 'horizontal'
      ? items.value.filter((item) => !item.parentKey)
      : items.value,
  )
  ...
  const change = (keys: readonly string[]) => {
    ...
    options.onChange(
      keys,
      items.value.filter((item) => activeSet.has(item.key)),
    )
  }
  ...
  const activate = (item: AnchorRegisteredItem) => {
    ...
    change(
      options.activeMode() === 'progress'
        ? visibleItems.value.slice(0, index + 1).map((entry) => entry.key)
        : [item.key],
    )
    clickScrollGuard.lock()
    container.scrollTo({
      top: Math.max(
        getAnchorTargetTop(target, container) - (item.targetOffset ?? options.targetOffset()),
        0,
      ),
  ...
  watch(options.activeKeys, (keys) => {
    controller.updateOptions({
      ...(keys === undefined ? {} : { activeKeys: keys }),
      defaultActiveKeys: options.defaultActiveKeys(),
    })
