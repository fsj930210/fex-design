...
>
  import { dataTableSortButtonClassName } from "@fex-design/components-styles/data-table";
  ...
  let {
    column,
    children,
  }: { column: Column<TFeatures, TData>; children?: import("svelte").Snippet } =
    $props();
  const direction = $derived(
    (column as unknown as SortableColumn).getIsSorted(),
  );
  const sortIndex = $derived(
    (column as unknown as SortableColumn).getSortIndex(),
  );
</script>
...
<Button
  ...
  onclick={(event) =>
    (column as unknown as SortableColumn).getToggleSortingHandler()?.(event)}
>
  ...
  <span class="inline-flex items-center" aria-hidden="true">
    ...
    {:else}
      <span class="relative inline-block size-3.5"
        ><ChevronRightIcon
          class="absolute inset-x-0 top-0 size-3 -rotate-90"
        /><ChevronRightIcon
          class="absolute inset-x-0 bottom-0 size-3 rotate-90"
        /></span
      >
    {/if}
