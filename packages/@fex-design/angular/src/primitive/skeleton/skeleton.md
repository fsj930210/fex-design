# Angular Primitive Skeleton

Styled, independently composable Skeleton placeholders. Every component uses a native `div` host.

## Import

    import { SkeletonAvatar, SkeletonBlock, SkeletonButton, SkeletonImage, SkeletonInput, SkeletonText } from '@fex-design/angular/primitive/skeleton'
...
| native attributes | native div bindings | — | Native div attributes and events pass through, including `class`, `style`, `data-*`, and `aria-*`. |
| element access | framework-native API | — | The component host is the native `div` and can be accessed with Angular queries. |
