...
import {
  ...
  anchorRailClassName,
  anchorRootClassName,
} from '@fex-design/components-styles/anchor'
...
export class AnchorRoot {
  ...
  readonly container = input<
    Window | HTMLElement | (() => Window | HTMLElement | null | undefined) | undefined
  >()
  ...
  readonly change = output<{
    ...
    items: readonly AnchorRegisteredItem[]
  }>()
  ...
  readonly highlightedKeys = computed(() => {
    ...
    for (const item of this.items()) {
      ...
      while (parentKey) {
        ...
        parentKey = this.itemMap.get(parentKey)?.parentKey
      }
    }
    return result
  })
  protected readonly hostClassName = createHostClassName(() =>
    anchorRootClassName({ orientation: this.orientation() }),
  )
  constructor() {
    effect(() => {
    ...
    afterNextRender(() => {
      ...
      const schedule = () => {
        ...
        frame = requestAnimationFrame(() => this.refresh())
      }
      const handleScroll = () => {
        if (this.clickScrollGuard.shouldHandleScroll()) schedule()
      }
      schedule()
      ...
      this.destroyRef.onDestroy(() => {
        ...
        this.clickScrollGuard.dispose()
      })
    })
  }
  registerItem(item: AnchorRegisteredItem) {
    this.itemMap.set(item.key, item)
    this.items.set([...this.itemMap.values()])
  ...
  activate(item: AnchorRegisteredItem) {
    ...
    this.setKeys(
      this.activeMode() === 'progress'
        ? visible.slice(0, index + 1).map((entry) => entry.key)
        : [item.key],
    )
    this.clickScrollGuard.lock()
    container.scrollTo({
      top: Math.max(
        getAnchorTargetTop(target, container) - (item.targetOffset ?? this.targetOffset()),
        0,
      ),
  ...
  private visibleItems() {
    return this.orientation() === 'horizontal'
      ? this.items().filter((item) => !item.parentKey)
      : this.items()
  }
  ...
  private setKeys(keys: readonly string[]) {
    const previous = this.controller.getSnapshot().activeKeys
    if (previous.length === keys.length && previous.every((key, index) => key === keys[index]))
      return
    this.controller.change(keys, [])
    const selected = new Set(keys)
    this.change.emit({
      activeKeys: keys,
      items: this.items().filter((item) => selected.has(item.key)),
    })
  }
  private refresh() {
    const container = this.resolveContainer()
    const positions = this.visibleItems().flatMap((item) => {
      const target = resolveAnchorTarget(item.target)
...
@Directive({
  selector: 'ul[anchorList]',
  standalone: true,
  host: { 'data-slot': 'anchor-list', '[class]': 'hostClassName()' },
})
export class AnchorList {
  private readonly root = inject(AnchorRoot)
  protected readonly hostClassName = createHostClassName(() =>
    anchorListClassName({ orientation: this.root.orientation() }),
  )
}

@Directive({
  selector: 'li[anchorItem]',
  standalone: true,
  host: {
...
export class AnchorItem {
  ...
  readonly record = computed<AnchorRegisteredItem>(() => {
    ...
    return {
      ...
      ...(this.parent ? { parentKey: this.parent.value() } : {}),
    }
  })
  ...
  constructor() {
    afterNextRender(() => {
      const unregister = this.root.registerItem(this.record())
...
@Directive({
  selector: 'button[anchorLink]',
  standalone: true,
  host: {
    type: 'button',
    'data-slot': 'anchor-link',
    '[attr.data-anchor-key]': 'item.value()',
...
export class AnchorLink {
  private readonly root = inject(AnchorRoot)
  protected readonly item = inject(AnchorItem)
  protected readonly active = computed(() => this.root.currentKeys().includes(this.item.value()))
  protected readonly hostClassName = createHostClassName(() =>
    anchorLinkClassName({
      orientation: this.root.orientation(),
      active: this.root.highlightedKeys().has(this.item.value()),
    }),
  )
  protected activate(event: MouseEvent) {
    queueMicrotask(() => {
      if (!event.defaultPrevented) this.root.activate(this.item.record())
    })
  }
}

@Directive({
  selector: 'div[anchorRail]',
  standalone: true,
  host: { 'aria-hidden': 'true', 'data-slot': 'anchor-rail', '[class]': 'hostClassName()' },
})
export class AnchorRail {
  private readonly root = inject(AnchorRoot)
  protected readonly hostClassName = createHostClassName(() =>
    anchorRailClassName({ orientation: this.root.orientation() }),
  )
}

@Directive({
  selector: 'span[anchorIndicator]',
  standalone: true,
  host: {
...
export class AnchorIndicator {
  private readonly root = inject(AnchorRoot)
  protected readonly style = computed(() => this.root.inkStyles()[0] ?? {})
  protected readonly hostClassName = createHostClassName(() =>
    anchorIndicatorClassName({ orientation: this.root.orientation() }),
  )
}
