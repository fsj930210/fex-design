<script lang="ts">
  ...
  function grid(mode: "multiple" | "single", disabled = false) {
    const columns: ColumnDef<Features, Person>[] = [
      {
        id: mode === "multiple" ? "__select__" : "__radio__",
        header: mode === "multiple" ? "Select all" : "",
        size: 44,
      },
    ...
    return {
      ...
      table: createDataTable({
        ...
        enableRowSelection: disabled
          ? (row) => row.original.status !== "paused"
          : true,
      }),
    };
  }
  const grids = [
    { title: "Multiple + select all", ...grid("multiple") },
    { title: "Single", ...grid("single") },
    { title: "Conditional disabled rows", ...grid("multiple", true) },
  ];
...
{#snippet header(item: Header<Features, Person>, revision: number)}<span
    data-revision={revision}
    >{#if item.column.id === "__select__"}<Checkbox
      ...
      />{:else}{String(item.column.columnDef.header ?? "")}{/if}</span
  >{/snippet}
{#snippet cell(item: Cell<Features, Person>, revision: number)}<span
    data-revision={revision}
    >{#if item.column.id === "__select__"}<Checkbox
        checked={item.row.getIsSelected()}
        disabled={!item.row.getCanSelect()}
        onCheckedChange={(checked) => item.row.toggleSelected(checked === true)}
      />{:else if item.column.id === "__radio__"}<RadioGroup
        orientation="horizontal"
        value={item.row.getIsSelected() ? item.row.id : ""}
        onValueChange={() => item.row.toggleSelected(true)}
        ><Radio
          value={item.row.id}
          disabled={!item.row.getCanSelect()}
          aria-label={`Select row ${item.row.id}`}
        /></RadioGroup
      >{:else}{String(item.getValue() ?? "")}{/if}</span
  >{/snippet}
<DemoSection
  title="Row selection"
  ...
  ><div class="grid gap-3 xl:grid-cols-3">
    {#each grids as item (item.title)}<section class="space-y-1.5">
        <h3 class="text-sm font-medium text-foreground">{item.title}</h3>
        <DataTable table={item.table} {header} {cell} /><ReactiveTableText
          snapshot={item.table.dataTableSnapshot}
          text={(revision) => selectedText(item, revision)}
        />
      </section>{/each}
  </div></DemoSection
>
