...
export interface EmptyProps
  extends
    ParentProps<Omit<JSX.HTMLAttributes<HTMLDivElement>, 'title'>>,
...
function DefaultEmptyImage() {
  return (
    ...
    >
      ...
      />
      <path d="M20 28h8" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
    </svg>
  )
}
...
export function Empty(props: EmptyProps) {
  ...
  const image = () =>
    local.image === null ? null : typeof local.image === 'string' ? (
      <img src={local.image} alt="" />
    ) : (
      (local.image ?? <DefaultEmptyImage />)
    )
    ...
    >
      <Show when={image() != null || local.title != null || local.description != null}>
        <EmptyHeader class={local.classNames?.header} style={local.styles?.header}>
          <Show when={image()}>
            {(value) => (
              <EmptyMedia class={local.classNames?.image} style={local.styles?.image}>
                {value()}
              </EmptyMedia>
            )}
          </Show>
          <Show when={local.title != null}>
            <EmptyTitle class={local.classNames?.title} style={local.styles?.title}>
              {local.title}
            </EmptyTitle>
          </Show>
      ...
      <Show when={local.children != null}>
        <EmptyContent class={local.classNames?.content} style={local.styles?.content}>
          {local.children}
        </EmptyContent>
      </Show>
    </PrimitiveEmpty>
