...
`SelectRoot` 接收动态 `options`，Item 挂载时不注册。搜索值保持内部状态；`showSearch` 让共享 Input Trigger 可输入，`filterOption` 用于本地过滤，远程搜索则在 `onSearch` 后替换 `options`。`virtual` 在 Content 内执行真正的窗口化渲染。
