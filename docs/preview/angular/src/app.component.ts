...
@Component({ selector: '#root', imports: [NgComponentOutlet], changeDetection: ChangeDetectionStrategy.OnPush, templateUrl: './app.component.html' })
export class AppComponent {
  ...
  protected readonly example = examples[
  ...
  constructor() {
    addEventListener('message', (event) => { if (isPreviewHostMessage(event.data)) this.values.set(event.data.props) })
    queueMicrotask(() => {
