<script lang="ts">
  ...
  let value = $state<CascaderValue>(["zhejiang", "hangzhou", "xihu"]);
</script>
...
<Demo
  ...
  <div class="flex gap-2">
    <Button
      onclick={() =>
        (value = ["jiangsu", "suzhou", "industrial-park", "loufeng"])}
      >Set Suzhou</Button
    ><Button variant="outline" onclick={() => (value = undefined)}>Clear</Button
    >
  </div>
