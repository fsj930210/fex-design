...
原生契约：通过 `$attrs` 透传原生 input 属性和监听器。模板 ref 暴露 `focus()`、`blur()` 和 `select()`。
...
使用 `class`、`classNames` 和 Vue `StyleValue` 绑定。 hover、focus-within、data 属性和 `[aria-invalid=true]` 等状态使用选择器覆盖，CSS Variables 只保留可复用基础 token。
