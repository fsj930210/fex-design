<script lang="ts">
  ...
  let { children: triggerContent, title, content, class: className, style, classNames, styles, ...rest }: PopoverProps = $props();
    ...
    <PopoverPortal>
      ...
      >
        ...
        <div data-slot="popover-body" class={classNames?.content} style={styles?.content}>
          {#if typeof content === 'string'}{content}{:else}{@render content?.(state)}{/if}
        </div>
      </PopoverContent>
    </PopoverPortal>
  {/snippet}
