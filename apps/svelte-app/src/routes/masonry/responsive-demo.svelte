<script lang="ts">
  ...
  let widths = $state([720, 720]),
    counts = $state([0, 0]);
  function layout(index: number, value: MasonryLayoutDetail) {
    counts[index] = value.columnCount;
  }
</script>

<div class="grid gap-4">
  {#each configs as config, configIndex (config.title)}<section
      class="grid gap-1.5"
    >
      <div>
      ...
      <div
        ...
        style:width={`${widths[configIndex]}px`}
      >
        <MasonryRoot
          columns={config.columns}
          gap={12}
          onLayoutChange={(value) => layout(configIndex, value)}
          ><MasonryViewport
            >{#each masonryItems.slice(0, 6) as item, index (item.id)}<MasonryItem
                itemKey={item.id}
                {index}
                ><div
