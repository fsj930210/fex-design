...
export default function Example() {
  ...
  return (
    <div className="grid w-full max-w-2xl gap-10">
      <section className="grid gap-4"><h4>整体禁用</h4><Demo defaultValue={[40]} disabled /></section>
      <section className="grid gap-4"><h4>禁用指定 Thumb</h4><div className="flex flex-wrap gap-4">{disabledThumbs.map((checked, index) => <Checkbox key={index} checked={checked} onChange={() => toggle(index)}>Thumb {index + 1}</Checkbox>)}</div><Demo defaultValue={[20, 50, 80]} disabled={disabledThumbs} /></section>
    </div>
  )
}
