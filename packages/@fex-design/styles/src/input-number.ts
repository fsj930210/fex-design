export const inputNumberActionsClassName =
  'pointer-events-none flex h-full w-[calc(var(--input-icon-size)+0.25rem)] shrink-0 flex-col items-stretch gap-0 overflow-hidden border-l border-transparent opacity-0 !pr-0 transition-opacity group-hover/input-root:pointer-events-auto group-hover/input-root:border-border group-hover/input-root:opacity-100 group-focus-within/input-root:pointer-events-auto group-focus-within/input-root:border-border group-focus-within/input-root:opacity-100'
