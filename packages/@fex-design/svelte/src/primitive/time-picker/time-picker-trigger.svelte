<script lang="ts">
  ...
  let { class: className, allowClear = true, placeholder, invalid = false, prefix, suffix }: { class?: string | undefined; allowClear?: boolean; placeholder?: string; invalid?: boolean; prefix?: Snippet; suffix?: Snippet } = $props()
...
<PopoverTrigger>{#snippet children(slot)}<div use:slot.action role="button" tabindex="-1" class="inline-block" aria-haspopup="dialog" aria-expanded={slot.state.open} data-state={slot.state.open ? 'open' : 'closed'} onclick={event => bridge(slot.props.onclick, event as never)} onkeydown={event => bridge(slot.props.onkeydown, event as never)} onpointerenter={event => bridge(slot.props.onpointerenter, event as never)} onpointerleave={event => bridge(slot.props.onpointerleave, event as never)} onfocus={event => bridge(slot.props.onfocus, event as never)} onblur={event => bridge(slot.props.onblur, event as never)} oncontextmenu={event => bridge(slot.props.oncontextmenu, event as never)}><InputRoot value={text} disabled={context.disabled()} readOnly={context.readOnly()} onValueChange={input} onClear={clear}>{#if prefix}<InputPrefix>{@render prefix()}</InputPrefix>{/if}<InputControl aria-invalid={invalid || undefined} aria-label="Time" placeholder={placeholder ?? context.format()} />{#if allowClear}<InputClear />{/if}{#if !allowClear || !text}<InputSuffix>{#if suffix}{@render suffix()}{:else}<ClockIcon class="size-4" />{/if}</InputSuffix>{/if}</InputRoot></div>{/snippet}</PopoverTrigger>
