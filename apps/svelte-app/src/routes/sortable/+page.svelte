    ...
    <header class="space-y-4">
      ...
      <div>
        ...
        <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Quick sortable components for common lists, plus useSortable for table
          columns and other custom layouts.
        </p>
      </div>
    </header>
    ...
    <div class="space-y-4">
      ...
      >
        <SortableRoot
          items={tasks}
          axis="y"
          onChange={(items) => (tasks = items as string[])}
        >
          {#snippet children({ items })}
            ...
            {/each}
            <SortableOverlay
              class="flex min-h-12 items-center gap-1.5 px-2 text-sm font-medium"
            >
      ...
      >
        <div class="grid gap-2 md:grid-cols-2">
          {#each Object.entries(previewPanels) as [containerId, items] (containerId)}
            <div
              use:panelContainer={containerId}
              class="min-h-56 rounded-md border border-border bg-background p-2"
            >
              <h2
                class="mb-2 text-sm font-medium capitalize text-muted-foreground"
              >
                {containerId}
              </h2>
              {#each items as item (item)}
                <div
                  use:panelItem={{ id: item, containerId }}
                  ...
                  style={styleToString(
                    (panelSnapshot.motionVersion,
                    panelSortable.getItemStyle(item)),
                  )}
                >
                  <span
                    class="grid size-7 place-items-center rounded-md bg-muted-background text-muted-foreground"
        ...
        {#if panelSnapshot.activeId}
          <div
            ...
            style={styleToString(
              (panelSnapshot.motionVersion, panelSortable.getOverlayStyle()),
            )}
          >
            <span
              class="grid size-7 place-items-center rounded-md bg-muted-background text-muted-foreground"
      ...
      >
        ...
        >
          <table class="w-full table-fixed border-collapse text-sm">
            <thead use:columnContainer
              ><tr
                >{#each previewColumns as column (column)}<th
                    use:columnItem={column}
                    ...
                    style={styleToString(
                      (columnSnapshot.motionVersion,
                      columnSortable.getItemStyle(column)),
                    )}
                    ><span class="inline-flex items-center gap-1.5"
                      ><span class="text-muted-foreground">::</span
                      >{columnLabels[column]}</span
                    ></th
                  >{/each}</tr
              ></thead
            >
            <tbody
              >{#each rows as row (row.email)}<tr
                  class="border-b border-border last:border-0"
                  >{#each previewColumns as column (column)}<td
                      class="px-2 py-1.5"
                      style={styleToString(
                        (columnSnapshot.motionVersion,
                        {
                          ...columnSortable.getMotionStyle(column),
                          visibility:
                            columnSnapshot.activeId === column
                              ? "hidden"
                              : undefined,
                        }),
                      )}>{row[column as ColumnKey]}</td
                    >{/each}</tr
                >{/each}</tbody
            >
          </table>
        ...
        {#if columnSnapshot.activeId}
          <div
            ...
            style={styleToString(
              (columnSnapshot.motionVersion,
              { ...columnSortable.getOverlayStyle(), height: "auto" }),
            )}
          >
            <div
            ...
            {#each rows as row (row.email)}
              <div class="border-b border-border px-2 py-1.5 last:border-0">
                {row[columnSnapshot.activeId as ColumnKey]}
              </div>
            {/each}
          </div>
        {/if}
