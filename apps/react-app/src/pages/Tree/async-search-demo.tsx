...
export function AsyncSearchTreeDemo() {
  const [keyword, setKeyword] = useState(''),
    [results, setResults] = useState<DemoTreeSearchResult[]>([]),
    [treeData, setTreeData] = useState<DepartmentNode[]>([]),
    [selectedKey, setSelectedKey] = useState<string>(),
    [expandedKeys, setExpandedKeys] = useState<readonly (string | number)[]>([])
  ...
  const choose = (result: DemoTreeSearchResult) => {
    setSelectedKey(result.node.id)
    setKeyword('')
    void getDemoTreeSubtree(result.node.id).then((response) => {
      setTreeData(convert(response.treeData))
      setExpandedKeys(response.expandedKeys)
    })
  }
  const clear = () => {
    setKeyword('')
  ...
  return (
    <TreeDemoSection
      ...
      description="The real server returns flat path-aware matches; selecting one loads and displays its ancestor subtree."
    >
      <InputRoot
        value={
          keyword ||
          (selectedKey
            ? (results.find((item) => item.node.id === selectedKey)?.node.name ?? selectedKey)
            : '')
        }
      ...
      {keyword ? (
        <ListboxRoot items={results} getItemValue={(item) => item.node.id} className="max-w-xl">
          {results.map((result) => (
            <ListboxItem
              key={result.node.id}
        ...
        <DemoTree
          ...
          features={[expansionFeature<DepartmentNode>(), selectionFeature<DepartmentNode>()]}
        />
      ) : null}
    </TreeDemoSection>
  )
