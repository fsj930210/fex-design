<script lang="ts">
  ...
  const lastDays = (days: number): CalendarRange<CalendarValue> => ({
  ...
  function setMultiple(next: unknown) {
    multiple = Array.isArray(next) ? ([...next] as CalendarValue[]) : [];
  }
  function setRange(next: unknown) {
    range = next as CalendarRange<CalendarValue>;
  }
  function setDynamicRange(next: unknown) {
    dynamicRange = next as CalendarRange<CalendarValue>;
  }
  function setSubmitted(next: unknown) {
    submitted = next as CalendarRange<CalendarValue>;
  }
  function dynamicDisabled(date: CalendarDate, part: "start" | "end") {
    if (part === "start") {
      ...
      return (
        isBeforeDate(date, endDate.subtract({ days: 6 })) ||
    ...
    return (
      ...
      isAfterDate(date, startDate.add({ days: 6 }))
    );
  }
</script>
...
<main class="min-h-screen bg-secondary-background px-2 md:px-6 py-4">
  <div class="mx-auto w-full max-w-5xl space-y-4">
    ...
    <div class="space-y-4">
      <Card
        title="基本用法"
        description="非受控 DatePicker 选择后立即提交并关闭 Popover。"
        class={{ content: demoContentClass }}
        ><DemoDatePicker defaultValue={today} placeholder="请选择日期" /></Card
      >
      <Card
        title="受控与非受控"
      ...
      <Card
        ...
        /><DemoDatePicker
          disabledDate={(date) => date.dayOfWeek === 6}
        /><DemoDatePicker disabled /></Card
      >
      <Card
        title="多选"
        ...
        class={{ content: demoContentClass }}
        ><div>
          <DemoDatePicker multiple value={multiple} onChange={setMultiple} />
          <p class="mt-1.5 w-full text-xs text-muted-foreground">
            当前值：{multiple.length
              ? multiple.map(getCalendarValueKey).join(", ")
              : "未选择"}
          </p>
        </div></Card
      >
      <Card
        ...
        class={{ content: demoContentClass }}
        ><div>
          <DemoDatePicker range value={range} onChange={setRange} />
          <p class="mt-1.5 w-full text-xs text-muted-foreground">
            当前范围：{range.start ? getCalendarValueKey(range.start) : "空"} ~ {range.end
              ? getCalendarValueKey(range.end)
              : "空"}
          </p>
        </div></Card
      >
      <Card
        ...
        ><div>
          ...
          />
          <p class="mt-1.5 w-full text-xs text-muted-foreground">
            当前范围：{dynamicRange.start
              ? getCalendarValueKey(dynamicRange.start)
              : "空"} ~ {dynamicRange.end
              ? getCalendarValueKey(dynamicRange.end)
              : "空"}
          </p>
        </div></Card
      >
      <Card
        ...
        ><DemoDatePicker
          picker="date"
          placeholder="请选择日期"
        /><DemoDatePicker picker="week" placeholder="请选择周" /><DemoDatePicker
          picker="month"
          placeholder="请选择月份"
        /><DemoDatePicker
          picker="quarter"
          placeholder="请选择季度"
        /><DemoDatePicker picker="year" placeholder="请选择年份" /></Card
      >
      <Card
        title="格式化"
      ...
      <Card
        ...
        class={{ content: demoContentClass }}
        ><DemoDatePicker range /><DemoDatePicker
          range
          picker="week"
        /><DemoDatePicker range picker="month" /><DemoDatePicker
          range
          picker="quarter"
        /><DemoDatePicker range picker="year" /></Card
      >
      <StatusDemos />
      <CustomDemos />
      <Card
        ...
        ><div class="space-y-1.5">
          ...
          <DemoDatePicker
            ...
            onChange={(next) => setDate(next, "panel")}
          />
        </div></Card
      >
      <Card
        title="允许留空"
        description="RangePicker 可允许清空某一端，适合“至今”等场景。"
        class={{ content: demoContentClass }}
        ><DemoDatePicker range allowEmpty={{ end: true }} /></Card
      >
      <IntegrationDemos />
    </div>
  </div>
