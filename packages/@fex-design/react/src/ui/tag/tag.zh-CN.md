...
| 内容 | `children?: ReactNode`; `closeIcon?: ReactNode` | — | 标签内容以及可选的关闭图标替换。 |
| 关闭事件 | `onClose?: MouseEventHandler<HTMLButtonElement>` | — | 通知自动生成的关闭按钮被触发。 |
...
React 使用 `children?: ReactNode`; `closeIcon?: ReactNode`。这是框架自身的内容 API，不引入跨框架的伪 Slots 抽象。
