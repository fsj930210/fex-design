...
| content | default slot | — | Content used when `count` is absent. |
| native attributes | Vue span attrs | — | attrs and exposed element behavior. |
...
`BadgeGroup` accepts `maxCount` and default slot plus scoped `overflow` slot with `{ count, items }`. Without `maxCount`, all items remain visible; the default overflow node is `+N`.

## BadgeRibbon API

`BadgeRibbon` accepts `color`, logical `placement="start" | "end"`, and default slot. Default placement is `end`; default color is `primary`.
