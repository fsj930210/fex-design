<script lang="ts">
  ...
  const asUlProps = (props: HTMLAttributes<HTMLDivElement>) =>
    props as unknown as HTMLAttributes<HTMLUListElement>;
  const asLiProps = (props: HTMLAttributes<HTMLDivElement>) =>
    props as unknown as HTMLAttributes<HTMLLIElement>;
  const asSectionProps = (props: HTMLAttributes<HTMLDivElement>) =>
...
>
  <TabsRoot defaultValue="files">
          ...
          <TabsItem value="files">
            {#snippet render({ props: itemProps, state, itemRef })}
              <li
                {...asLiProps(itemProps)}
                use:itemRef
                class={cn(itemProps.class, itemClassName)}
              >
                Files
    ...
    <TabsContent value="files">
      {#snippet render({ props })}<section
          {...asSectionProps(props)}
