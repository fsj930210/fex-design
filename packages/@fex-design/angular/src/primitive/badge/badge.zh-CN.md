...
| 组件 | 宿主 | 说明 |
| --- | --- | --- |
| Badge | `badge` | 展示短文本或计数，支持语义色和自定义颜色。 |
| BadgeDot | `span[badgeDot]` | 展示不带数字内容的独立状态小圆点。 |
| BadgeGroup | `badge-group` | 排列徽标，并可用 `maxCount` 收起为 `+N`。 |
| BadgeRibbon | `span[badgeRibbon]` | 展示独立 Ribbon 标记；包含块由使用方提供。 |
...
| 内容 | content projection | — | 未传 `count` 时展示的内容。 |
| 原生属性 | native host bindings | — | native host attributes and `element`。 |
...
`BadgeGroup` 接受 `maxCount`，集合内容使用 projected Badge children plus optional overflow TemplateRef。不传 `maxCount` 时显示全部项目，默认溢出节点为 `+N`。

## BadgeRibbon API

`BadgeRibbon` 接受 `color`、逻辑位置 `placement="start" | "end"` 和 content projection。默认位置为 `end`，默认颜色为 `primary`。
