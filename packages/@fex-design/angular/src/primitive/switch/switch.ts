...
import { switchClassName, switchContentClassName, switchThumbClassName } from '@fex-design/styles/switch'
...
@Component({
  selector: 'button[switch]', standalone: true, changeDetection: ChangeDetectionStrategy.OnPush,
  host: {
    type: 'button', role: 'switch', '[disabled]': 'disabled() || loading()',
    '[attr.aria-checked]': 'currentChecked()', '[attr.aria-busy]': 'loading() || null',
    '[attr.data-state]': 'state()', '[attr.data-disabled]': "disabled() ? '' : null",
    '[attr.data-loading]': "loading() ? '' : null", '[attr.data-size]': 'size()',
    '[attr.data-shape]': 'shape()', '[class]': 'hostClassName()', '(click)': 'handleClick($event)',
    'data-slot': 'switch',
  },
  template: '<ng-content />',
})
export class SwitchRoot {
  ...
  protected readonly hostClassName = createHostClassName(() => cn(switchClassName({ size: this.size(), shape: this.shape() })))
...
@Component({
  selector: 'span[switchThumb]', standalone: true, changeDetection: ChangeDetectionStrategy.OnPush,
...
@Component({
  selector: 'span[switchContent]', standalone: true, changeDetection: ChangeDetectionStrategy.OnPush,
  host: { '[attr.data-state]': 'state()', '[class]': 'hostClassName()', 'aria-hidden': 'true', 'data-slot': 'switch-content' },
  template: '<ng-content />',
})
