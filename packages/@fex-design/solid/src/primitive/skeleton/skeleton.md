# Solid Primitive Skeleton

Styled, independently composable Skeleton placeholders. Every component uses a native `div` host.

## Import

    import { SkeletonAvatar, SkeletonBlock, SkeletonButton, SkeletonImage, SkeletonInput, SkeletonText } from '@fex-design/solid/primitive/skeleton'
...
| native attributes | `JSX.HTMLAttributes<HTMLDivElement>` | — | Native div attributes and events pass through, including `class`, `style`, `data-*`, and `aria-*`. |
| element access | framework-native API | — | `ref` exposes the native `HTMLDivElement`. |
