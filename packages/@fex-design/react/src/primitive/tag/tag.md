...
| content | `children?: ReactNode` | — | Tag content. |
...
| content | `children?: ReactNode` | CloseIcon | Completely replaces the default close icon when provided. |
| events | `onClick` / native mouse events | — | Native button events; Primitive adds no action-specific event. |
| native attributes | ComponentProps<'button'> | — | Native button attributes and events. |
...
Content follows React's native model: `children?: ReactNode`. Place `TagAction` inside `Tag` for an inline operation. It defaults to CloseIcon, while custom content can represent remove, edit, more, or another action. `TagAction` does not prescribe behavior or coordinate the parent disabled state.
