    ...
    <header class="space-y-4">
      ...
      <div>
        ...
        <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Boolean checks, grouped layouts, indeterminate state, disabled state,
          and aria-invalid styling.
        </p>
      </div>
    </header>
    ...
    <div class="space-y-4">
      ...
      >
        <div class="grid min-w-0 gap-2">
          <div class="inline-flex items-center gap-2 text-sm">
            <Checkbox defaultChecked /><span>Enable notifications</span>
          </div>
          <div class="inline-flex items-center gap-2 text-sm">
            <Checkbox aria-invalid="true" /><span
              >Invalid style from aria-invalid</span
            >
          </div>
        </div>
      </Card>
      ...
      >
        <div class="grid min-w-0 gap-2">
          <div class="inline-flex items-center gap-2 text-sm">
            <Checkbox
              checked={controlled}
              onCheckedChange={(checked) => (controlled = checked)}
            />
            <span>Controlled checkbox</span>
          </div>
          <p class="text-sm text-muted-foreground">
            Current value: {String(controlled)}
          </p>
        </div>
      </Card>
          ...
          <div class="inline-flex items-center gap-2 text-sm">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                (partialValue = checked === true ? optionValues : [])}
            />
      ...
      >
        <CheckboxGroup>
          ...
          <div class="inline-flex items-center gap-2 text-sm">
            <Checkbox disabled defaultChecked /><span>Disabled checked</span>
          </div>
        </CheckboxGroup>
      </Card>
    </div>
