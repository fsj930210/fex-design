...
import { createContext, use, useEffect, useState, useSyncExternalStore, type ComponentProps } from 'react'
...
export function Avatar({ size = 'md', shape = 'circle', className, children, ...props }: ComponentProps<'span'> & AvatarStyleProps) {
  const [controller] = useState(() => createImageLoadingController(loadImage))
  useEffect(() => () => controller.reset(), [controller])
  return <AvatarContext value={{ controller }}><span {...props} data-slot="avatar" data-size={size} data-shape={shape} className={cn(avatarClassName({ size, shape }), className)}>{children}</span></AvatarContext>
}
...
export function AvatarImage({ src, className, onLoadingStatusChange, ...props }: AvatarImageProps) {
  ...
  const status = useSyncExternalStore(controller.subscribe, controller.getStatus, () => 'idle' as ImageLoadingStatus)
  useEffect(() => {
    if (src) controller.load({
      src,
  ...
  return status === 'loaded' ? <img {...props} src={src} data-slot="avatar-image" className={cn(avatarImageClassName, className)} /> : null
}
...
export function AvatarFallback({ delayMs, className, children, ...props }: AvatarFallbackProps) {
  ...
  const status = useSyncExternalStore(context.controller.subscribe, context.controller.getStatus, () => 'idle' as ImageLoadingStatus)
  ...
  return canRender && status !== 'loaded' ? <span {...props} data-slot="avatar-fallback" className={cn(avatarFallbackClassName, className)}>{children}</span> : null
}

export function AvatarBadge({ className, children, ...props }: ComponentProps<'span'>) {
  return <span {...props} data-slot="avatar-badge" className={cn(avatarBadgeClassName, className)}>{children}</span>
}

export function AvatarGroup({ className, children, ...props }: ComponentProps<'div'>) {
  return <div {...props} role="group" data-slot="avatar-group" className={cn(avatarGroupClassName, className)}>{children}</div>
}

export function AvatarGroupCount({ className, children, ...props }: ComponentProps<'span'>) {
  return <span {...props} data-slot="avatar-group-count" className={cn(avatarGroupOverflowClassName, className)}>{children}</span>
}
