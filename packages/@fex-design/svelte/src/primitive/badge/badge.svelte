<script lang="ts">
  ...
  let { class: className, children, color, count, showZero = false, overflowCount, style, ...rest }: Props = $props()
  ...
  const classList = $derived(cn(badgeClassName({ color: presetColor }), className))
  const mergedStyle = $derived([style, customColor ? `--badge-color:${customColor}` : ''].filter(Boolean).join(';'))
</script>
{#if visible}<span {...rest} data-slot="badge" data-color={color ?? 'default'} class={classList} style={mergedStyle || undefined}>{#if value !== undefined}{value}{:else}{@render children?.()}{/if}</span>{/if}
