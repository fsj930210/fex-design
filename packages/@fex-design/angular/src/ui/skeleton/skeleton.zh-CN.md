...
| SkeletonPlaceholder | 标记自定义骨架的 `ng-template` 指令。 |
...
| 原生属性 | 原生 div 绑定 | — | 骨架状态下透传到根 div；`class`、`style` 与 root 结构化配置合并。 |
...
内容投影提供真实内容；`<ng-template skeletonPlaceholder>` 提供自定义骨架，并需导入 `SkeletonPlaceholder`。
