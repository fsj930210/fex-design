...
{#snippet cell(item: Cell<Features, Person>)}
  {#if item.column.id === "name"}<span class="inline-flex items-center gap-2"
      >{#if item.row.getIsPinned()}<Badge variant="outline"
          >Pinned {item.row.getIsPinned()}</Badge
        >{/if}{item.row.original.name}</span
    >{:else if item.column.id === "pin"}<span class="inline-flex gap-1"
      ><Button size="sm" variant="outline" onclick={() => item.row.pin("top")}
        >Top</Button
      ><Button size="sm" variant="outline" onclick={() => item.row.pin(false)}
        >Center</Button
      ><Button
        size="sm"
        variant="outline"
        onclick={() => item.row.pin("bottom")}>Bottom</Button
      ></span
    >{:else}{item.row.original.status}{/if}
{/snippet}
<DemoSection
  title="Row pinning"
  ...
  ><section class="space-y-1.5">
    <h3 class="text-sm font-medium text-foreground">
      Top, scrollable center and bottom
    </h3>
    <DataTable {table} {cell} class={{ viewport: "max-h-56" }} />
  </section></DemoSection
>
