...
组合 Primitive 的快捷浮层，提供标题、内容和语义样式。

## 导入

```ts
import { Popover } from '@fex-design/angular/ui/popover'
```

## 框架契约

使用原生元素属性选择器与 signal inputs/outputs。触发器为 button[popoverTrigger]，延迟内容放入 ng-template[popoverPortal]。UI 内容支持字符串或 TemplateRef。
...
classNames 与 styles 支持 root、title、content、arrow。root 指浮层面板；样式值使用本框架原生表达，Svelte 使用 CSS 字符串。
