...
defineOptions({ name: 'AnchorIndicator', inheritAttrs: false })
const attrs = useAttrs()
const anchor = inject(anchorContextKey)
if (!anchor) throw new Error('AnchorIndicator must be used inside AnchorRoot')
</script>
<template>
  <span
    v-for="(inkStyle, index) in anchor.inkStyles.value"
    v-bind="attrs"
    :key="index"
    data-slot="anchor-indicator"
    :class="
      cn(
        anchorIndicatorClassName({ orientation: anchor.orientation.value }),
