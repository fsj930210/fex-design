...
export function MasonryPage() {
  return (
    <main className="grid gap-4 p-2 md:p-6">
      ...
      <Card title="动态增删与排序" description="覆盖添加、删除、中间插入、打乱和恢复，所有项目使用稳定 key。"><CollectionMasonryDemo /></Card>
      <Card title="响应式列数" description="同时验证最小列宽和容器断点；拖动控制的是组件容器宽度，不依赖窗口断点。"><ResponsiveMasonryDemo /></Card>
      <Card title="排布策略与指定列" description="同一数据对照 ordered、shortest，并展示自动项目与固定列项目混排。"><StrategyMasonryDemo /></Card>
      <Card title="RTL 与隐藏容器恢复" description="验证 inline-start 方向、DOM 顺序，以及容器宽度从 0 恢复后的重新布局。"><RtlHiddenMasonryDemo /></Card>
      <Card title="虚拟瀑布流" description="5,000 条数据使用 TanStack Virtual lanes；展示挂载量、动态测量和滚动定位。"><VirtualMasonryDemo /></Card>
    </main>
  )
}
