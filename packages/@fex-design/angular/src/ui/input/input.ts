...
import type {
  InputClassNames as InputClassNamesBase,
  InputSearchMeta,
  InputSize,
  InputStyles as InputStylesBase,
  InputVariant,
...
abstract class InputBase {
  ...
  protected template(value: string | TemplateRef<unknown> | null | undefined) {
    return typeof value === 'string' || value == null ? null : value
  }
  protected text(value: string | TemplateRef<unknown> | null | undefined) {
...
@Component({
  ...
  host: { '[class]': 'groupClassName', 'data-slot': 'input-group', role: 'group' },
...
export class InputSearch extends InputBase {
  ...
  protected runSearch(source: InputSearchMeta['source']) {
    if (!this.loading())
      this.search.emit({ value: this.root()?.currentValue() ?? '', meta: { source } })
  }
  protected keydown(event: KeyboardEvent) {
    if (event.key === 'Enter' && !event.defaultPrevented) this.runSearch('enter')
  }
}
