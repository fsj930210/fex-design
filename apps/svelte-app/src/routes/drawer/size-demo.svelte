...
<div class="space-y-3">
  <RadioGroup
    ...
    onValueChange={(value) => (placement = value as DrawerPlacement)}
  >
    {#each placements as item}<label
        class="inline-flex items-center gap-2 text-sm text-foreground"
        ><Radio value={item} /><span>{item}</span></label
      >{/each}
  </RadioGroup>
  <Drawer {placement} {size} resizable onSizeChange={(value) => (size = value)}
    ><DrawerTrigger
      >{#snippet children(props)}<Button {...props}
          >Resizable {placement} ({Math.round(size)}px)</Button
        >{/snippet}</DrawerTrigger
    ><DrawerPortal
      ><DrawerMask /><DrawerContent aria-label="Resizable drawer"
        ><DrawerResizeHandle /><DrawerHeader
          >Resizable {placement} drawer<DrawerClose /></DrawerHeader
        ><DrawerBody
          >Drag the inner edge to change {placement === "left" ||
          placement === "right"
            ? "width"
            : "height"}.</DrawerBody
        ></DrawerContent
