...
`bind:ref` 暴露原生 `HTMLInputElement`。

## 样式扩展

使用 `class`、`classNames` 和原生 style 字符串。 使用 `classNames` / `styles` 定位语义区域，使用选择器覆盖 hover、focus 和错误状态；可复用主题使用 `--input-background`、`--input-border-color`、`--input-color`、`--input-placeholder-color`、`--input-ring-color` 和 `--input-group-radius` 等基础变量。
