...
export function ControlledDemo() {
  ...
  return (
    <Card title="受控" description="open 和 current 完全由父组件管理。">
      <TourRoot open={open()} current={current()} onOpenChange={setOpen} onChange={setCurrent}>
        ...
        <TourPortal>
          <TourOverlay />
          <TourStep name="controlled-first" target="controlled-first">
            <TourContent>
              <TourArrow />
              <TourPanel title="受控第 1 步" description="open 和 current 都由外部状态管理。">
                <DefaultTourActions />
              </TourPanel>
            </TourContent>
          </TourStep>
          <TourStep name="controlled-second" target="controlled-second">
            <TourContent>
              <TourArrow />
              <TourPanel title="受控第 2 步" description="导航变化通过回调通知父组件。">
                <DefaultTourActions />
              </TourPanel>
            </TourContent>
          </TourStep>
