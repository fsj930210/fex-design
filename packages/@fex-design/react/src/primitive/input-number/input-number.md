...
Control inherits `ComponentProps<'input'>`; native focus, blur, keyboard, aria, data attributes, and element access pass through.

## useInputNumber API

The public logic layer accepts the same options and drives custom DOM without rendering the default structure. Reactive values use plain values.
