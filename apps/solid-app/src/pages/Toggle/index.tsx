    ...
    <main class="min-h-screen bg-secondary-background px-2 md:px-6 py-4">
      <div class="mx-auto w-full max-w-5xl space-y-4">
        ...
        <div class="grid gap-4">
          <Card title="Standalone" description="A single Toggle exposes a clear pressed state without needing a group."><div class="grid justify-items-start gap-2"><Toggle pressed={pressed()} onChange={setPressed}><span aria-hidden>★</span>Favorite</Toggle><p class="text-sm text-muted-foreground">{pressed() ? 'Added to favorites' : 'Not in favorites'}</p></div></Card>
          ...
          <Card title="Multiple selection" description="Multiple mode keeps related formatting controls inside one visual surface."><div class="grid gap-2"><ToggleGroup multiple value={formats()} onChange={setFormats} aria-label="Text formatting"><Toggle value="bold"><strong aria-hidden>B</strong>Bold</Toggle><Toggle value="italic"><em aria-hidden>I</em>Italic</Toggle><Toggle value="underline"><u aria-hidden>U</u>Underline</Toggle></ToggleGroup><p class="text-sm text-muted-foreground">Active: {formats().join(', ') || 'none'}</p></div></Card>
