...
import type { MasonryLayoutDetail } from '@fex-design/vue/primitive/masonry'
...
const items = masonryItems.slice(0, 9),
  ordered = ref<Record<string, number>>({}),
  shortest = ref<Record<string, number>>({})
...
<template>
  <div class="grid gap-4">
    <section
      v-for="placement in ['ordered', 'shortest'] as const"
      :key="placement"
      class="grid gap-1.5"
    >
      <div>
      ...
      <MasonryRoot
        ...
        :placement="placement"
        :on-layout-change="
          (value) => (placement === 'ordered' ? (ordered = cols(value)) : (shortest = cols(value)))
        "
        ><MasonryViewport
          ><MasonryItem
            ...
            >
              ...
              <p class="text-xs text-muted-foreground">
                输入序号 {{ index + 1 }} · 最终第
                {{ (placement === 'ordered' ? ordered : shortest)[item.id] ?? '…' }} 列
              </p>
            </div></MasonryItem
          ></MasonryViewport
        ></MasonryRoot
