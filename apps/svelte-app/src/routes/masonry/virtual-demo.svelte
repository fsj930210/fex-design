<script lang="ts">
  ...
  import { virtualMasonryItems, type MasonryDemoItem } from "./data";
  let host: HTMLDivElement,
    expanded: string[] = $state([]),
    mounted = $state(0),
    offset = $state(0);
  const update = (element: HTMLElement) => {
    offset = element.scrollTop;
    mounted = element.querySelectorAll("[data-index]").length;
  };
  const scrollTo = (index: number) => {
    ...
    if (viewport)
      viewport.scrollTo({
        top:
          (index / virtualMasonryItems.length) *
...
{#snippet content(item: MasonryDemoItem, index: number)}<article
    class="rounded-md border border-border bg-background p-2"
    style:min-height={`${expanded.includes(item.id) ? item.height + 100 : item.height}px`}
  >
...
<div bind:this={host} class="grid gap-2">
  <div class="flex flex-wrap items-center gap-1.5">
    <span class="text-sm"
      >总数 5,000 · 当前挂载 {mounted || "…"} · scrollTop {Math.round(
        offset,
      )}</span
